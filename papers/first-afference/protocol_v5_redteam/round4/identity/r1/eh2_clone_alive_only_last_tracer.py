"""EXAM_HOLE: every exam CLONE_ALIVE case (X57, X58) runs a single tracer. A mutant that runs the
CLONE_ALIVE check only in the tracer that releases the mint (the last holder) passes the frozen
exam. Paired harness: nested tracers on one target; the inner trace calls ONLY a same-globals
clone kept alive past the inner exit and dropped before the outer exit. Original: the inner trace
refuses CLONE_ALIVE. Mutant: the inner gate PASSES on the clone call."""
MUT_NAME = 'CLONE_ALIVE checked only by the tracer that releases the mint'
OLD = '                if sys.getrefcount(m.code) - expected > 0:'
NEW = '                if not m.tracers and sys.getrefcount(m.code) - expected > 0:'
EXPECT_ORIG = 'REFUSED [V5:CLONE_ALIVE]'
HARNESS = 'import json, os, subprocess, sys, tempfile, textwrap, types\nfrom pathlib import Path\nsys.path.insert(0, os.environ["RT4_STYXX_ROOT"])\nimport styxx.protocol as P\nfrom styxx.protocol import Experiment, GateSpecError, coverage_trace\nassert Path(P.__file__).resolve().is_relative_to(Path(os.environ["RT4_STYXX_ROOT"]).resolve())\n_TMP = Path(tempfile.mkdtemp(prefix="rt4id_h_"))\nsys.path.insert(0, str(_TMP))\ndef write_mod(name, src):\n    (_TMP / f"{name}.py").write_text(textwrap.dedent(src), encoding="utf-8")\n    sys.modules.pop(name, None)\ndef make_exp(**gates):\n    d = Path(tempfile.mkdtemp(prefix="rt4id_repo_"))\n    g = {k: {"metric": "m", "op": ">=", "value": 0.5, **v} for k, v in gates.items()}\n    spec = {"gates": g, "outcomes": [{"when": {k: True for k in g}, "verdict": "PASS"},\n                                     {"when": {}, "verdict": "FAIL"}], "smoke_verdict": "S"}\n    p = d / "PREREG_case.md"\n    p.write_text("# case\\n\\n```gates\\n" + json.dumps(spec) + "\\n```\\n", encoding="utf-8")\n    for c in (["git", "init", "-q"], ["git", "add", "-A"],\n              ["git", "-c", "user.email=a@b", "-c", "user.name=a", "-c", "commit.gpgsign=false",\n               "commit", "-qm", "c"]):\n        subprocess.run(c, cwd=d, check=True)\n    return Experiment(p)\ndef score(exp, rec, m=1.0):\n    try:\n        v = exp.score({"m": m, "coverage_trace": rec})\n        return f"PASS {v.coverage}"\n    except GateSpecError as e:\n        return f"REFUSED {str(e)[:120]}"\nwrite_mod("rt4_nest", """\n    def f(x=0): return x\n""")\nimport rt4_nest as mn\nexp_outer = make_exp(O={"exercises": ["rt4_nest:f"]})\nexp_inner = make_exp(I={"exercises": ["rt4_nest:f"]})\nkeep = []\ndef inner_body():\n    clone = types.FunctionType(mn.f.__code__, mn.f.__globals__, "f_copy")   # same-globals clone\n    keep.append(clone)\n    clone(1)                                  # only the clone is called: credited as f\nwith coverage_trace(exp_outer) as outer:\n    with coverage_trace(exp_inner) as inner:\n        inner.run("I", inner_body)\n    keep.clear()                              # the clone dies after the INNER trace exited\n    outer.run("O", mn.f, 1)\nprint(score(exp_inner, inner.record()))\n'
