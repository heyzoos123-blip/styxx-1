#!/usr/bin/env python
"""EXAM_HOLE: __exit__ returns at once unless the state is "active". The mutant drops that guard,
so a second __exit__ (a harness that exits explicitly on success and again in a finally) runs the
exit a second time: it records a false CODE_SWAPPED (the code was already restored) and decrements the
global _ACTIVE again, which leaks into every later tracer in the process: with _ACTIVE at -1 the next
trace's hook removes itself at every event and that trace sees nothing. No exam case exits twice.
...
MUTANT_NAME = "E05_double_exit"
MUTATION = [('            if self._state != "active":\n                return False\n            self._state = "exiting"', '            self._state = "exiting"')]
...
HARNESS = r'''mod = module("rt4e05_fx", "def f(): return 1\n")
exp = experiment(G={"exercises": ["rt4e05_fx:f"]})
cov = coverage_trace(exp)
cov.__enter__()
try:
    cov.run("G", mod.f)
    cov.__exit__(None, None, None)          # explicit exit once the work succeeded ...
finally:
    cov.__exit__(None, None, None)          # ... and the cleanup path exits again
first = gate(exp, cov.record(), "G")
exp2 = experiment(H={"exercises": ["rt4e05_fx:f"]})
with coverage_trace(exp2) as cov2:          # a later, independent trace in the same process
    cov2.run("H", mod.f)
emit(first=first, second=gate(exp2, cov2.record(), "H"), active_after=P._ACTIVE)'''


def judge(orig, mut):
    ok = ("pass" in orig["first"] and "pass" in orig["second"] and orig["active_after"] == 0
          and mut["first"].get("refused", "").startswith("[V5:CODE_SWAPPED]")
          and mut["second"].get("refused", "").startswith("[V5:NOT_EXERCISED]"))
    return ok, ("a double __exit__ is harmless in the original (both traces PASS) but in the mutant records a "
                "false CODE_SWAPPED and leaves _ACTIVE at -1, so a LATER independent trace sees nothing and "
                "refuses NOT_EXERCISED" if ok else "no difference")
