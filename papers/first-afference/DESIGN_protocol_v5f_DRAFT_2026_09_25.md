...
_MON     = [None]   # (get_tool, get_events, set_events, set_local_events, register_callback, use_tool_id): bound by the first coverage_trace()
...
**Reload (revision 2, N8).** Every name above except `_TRACER_ID`, `_CACHE_WRAPPER`, `_BUSY_SECONDS`, `_LOCAL` and the revision-5 one-call names (`_map` through `_NAME1`, `_monotonic`, `_sleep`) is bound as `X = globals().get("X", <initial>)`, and `_TOOL_NAME` as `globals().get("_TOOL_NAME") or <fresh name>`. So `importlib.reload(styxx.protocol)`, which re-runs the module in the same globals dict, keeps the registries, the cut, the tool id and the tool name. Rebinding them instead had two effects in `rev2/p10_reload.py`: a mint held across the reload was lost and never retired, and each reload used up a tool id under a new name, so the second reload refused MONITOR_BUSY for the rest of the process. After a reload, the callbacks registered on the id are the old function objects, which read the same globals. The first exit's MONITOR_LOST test (M5 X5) finds a callback that is not the current function, re-registers the reloaded ones, and notes MONITOR_LOST once. That note lands on whichever trace exits first after the reload, which can be a trace entered after the reload that lost nothing (revision 3, N11). Notes never refuse, so the only effect is a misleading note on that one trace. A copy loaded under another module name has its own globals and stays a second copy (X142).
...
2. `_register(_TOOL[0])`: one call that registers the five callbacks iff the id carries `_TOOL_NAME` (M7);
3. if the id now carries `_TOOL_NAME`: `_LOST[0] = _LOST[0] + 1`.

No owner exists, so nobody is clobbered. The events and callbacks on the id are whatever its last owner left, because `set_events` on a freed id raises ValueError: `tool 4 is not in use` (`rev2/p3_free_reclaim.py`). After a plain free of styxx's id they are styxx's own, and the steps below clear them as usual. Revision 3 (N3) states the contrived case too: if another tool took the freed id and then freed it in turn, the leftovers are that tool's. Step 0 replaces its callbacks for styxx's five events, and the reconciliation's last step clears the global events only when PY_UNWIND is among them. Any other global or local events and callbacks that tool left stay on the id, now under styxx's name, until styxx's process exits. An id that another tool now holds is never touched (L-MONITOR).
...
2. `_set_local(tool, m.code, 0)`: one call that clears the local events iff the id carries `_TOOL_NAME` (revision 5; M7). An id that no longer carries this copy's name is never touched, not even by another tool freeing and re-taking it between a test and the write (L-MONITOR; X154).
3. `m.pend.clear()`.
...
- **E4. `_locked(_enter_txn, core, resolved)`,** which runs:
  1. reconcile;
  2. `_ensure_tool()`:
     - If `_TOOL[0]` is set: `_reclaim()` (M3 step 0), then keep the id if `_named(_TOOL[0])`. Revision 5: reclaiming here, not only in step 1's reconciliation, closes a gap the extended model check found. If styxx's id was freed after step 1 and before this step, revision 4 re-took it here silently, without counting it in `_LOST`. A section of another trace that opened meanwhile got no event and lost its raising calls, and that trace's exit then saw `_LOST` unchanged and the id named, so it noted nothing (`rev5/m5_modelcheck.py`, `ext: enter(X) || open(Y)`; X137e). `_named(i)` is `get_tool(i) is _TOOL_NAME` (revision 5): an identity test, which runs no `__eq__` at all. `use_tool_id` stores and `get_tool` returns the very object passed (`rev5/t1_onecall_atomic.py` part F), and `_TOOL_NAME` survives a reload (M1). Revision 4's `type(n) is str and n == _TOOL_NAME` also ran no foreign `__eq__` (N1, `critic4/a5_tool_name_eq.py`, X142b). The identity test is also what the one-call steps use.
     - Otherwise, for id 4 then 3:
       ...
       - then, if `_named(id)`: `_register(id)` (one call, gated on the name), and store `_TOOL[0]` last.
...
- **X5. The exit transaction.** `held, more = _locked(_exit_txn, core)`, which runs:
  ...
  2. MONITOR_LOST test. The loss is noted if any of these holds:
     ...
     - `not _named(tool)`: the id was freed and then taken by another tool. In that case nothing below is tested, and the id is never touched again: no callback is registered on it and no event is changed;
     ...
     - re-registering one of our callbacks returns a different previous callback. The re-registration is `_register(tool)`, one call gated on the name, which returns the previous callbacks, or none when the id is not named (revision 5). It can therefore overwrite only a callback some party placed on styxx's own id, and it repairs that callback for later traces. Revision 4 tested the name, then registered. A tool that freed and re-took the id between the two lost its callbacks (the fifth critic's N1, `critic5/c4_toolid_retake_race.py`; X154).
...
G_HYG checks the shape. `_unwind_on`, `_unwind_off`, `_take`, `_register` and `_set_local` each consist of reads of `_TOOL[0]` and `_MON[0]` and then that one statement. Each pipeline names only the M1 one-call names, `_MON`'s functions, `_ANCHORS.get`, `_ANCHORS.pop` and literal tuples. The mutation audit's revision-5 row names the witnesses for splitting any of them.
...
- `rev5/m5_modelcheck.py`: every interleaving of 35 configurations. Each C operation is one step, and each one-call step is one step. The configurations are revision 4's pairs and triples, plus:
  ...
  Revision 5 violates none of U1, A0, STALE, FALSEFLAG, LOSTNOTE, CLOBBER, P8, HANG, CYC_OPEN, END and REPAIR in 70 runs (4.9 million state hashes). Its only cycle is the robust mutex's through a user lock, broken by the bound (CYC_TX). Revision 4 in the same model shows every finding of the fifth critic (R19, the masked loss, the cycles, the tool-id race) and the greenlet loss. Each revision-5 rule's mutant violates a property, except two whose effects are not properties of the model. The Revision 5 section has the enumeration.
...
- **Sources:** signal-handler exceptions, KeyboardInterrupt, RecursionError, MemoryError, audit hooks, and a foreign tool raising at an event of one of our frames. That foreign tool can be a lower-id monitoring tool, or a settrace/setprofile function on the thread. Revision 5 (N2) lists every machinery site that raises an audit event, where a raising audit hook is such a source:
  ...
  No one-call step raises an audit event (`rev5/t1_onecall_atomic.py` part C). So revision 5 removes revision 4's two open- and close-path sites: `_txn()` in every clearing `_unwind_off`, and `_alive` in every waiting open (the fifth critic's `critic5/c5_audit_events.py`). `sys._current_frames` costs 3–3.8 µs per call at 51 threads (c5).
- **The GIL is assumed.** That is why free-threaded builds refuse.
- **Commits are single C operations:** dict setitem, `setdefault`, `pop` and `get` with str, int, frame, function, `_Core` or `_Txn` keys (identity or str hashing, so no Python `__hash__` or `__eq__` runs); `list.append`; `set.clear`; one STORE_ATTR or STORE_SUBSCR; one `sys.monitoring` call; or, from revision 5, one one-call step (M7), which is one C call however many operations it makes inside. The one comparison with a foreign object, another tool's name from `get_tool`, is an identity test (`_named`, revision 5), so no foreign `__eq__` runs. The machinery's correctness arguments never use the absence of an eval-breaker check between two bytecodes. They use the order of these operations on each thread, and the fact that nothing at all runs inside one C call (revision 5).
...
- **I5: no hang, and the only wait is the robust mutex (revision 5).** The mutex serializes transactions (enter and exit). A dead owner is stolen; a live owner on the same thread gives REENTRANT instead of a wait; a live owner on another thread is waited on for at most 10 s, by a clock bound at import (MACHINERY_BUSY). No threading lock exists, so a skipped release cannot hang anyone. Opens, closes and callbacks never wait for anything. Revision 4's opener wait is deleted.

  One cycle of waits can still form, and only through user code. The mutex owner runs user code inside its transaction that waits for a thread, and that thread is itself waiting for the mutex. Examples: an audit hook on a `__code__` write, or a finalizer, that takes a lock another thread holds around `cov.__exit__()` or `with coverage_trace(...)`. The waiter's bound breaks the cycle. After 10 s it raises MACHINERY_BUSY, its `with` releases the lock, and the owner proceeds (over-blocking #20).
...
- **Tool acquisition.** The callbacks are registered before `_TOOL[0]` is stored. A tool left named ours with missing callbacks is adopted and re-registered at the next enter.
  - Revision 5: no race with another tool can make styxx raise or write on a foreign id. Revision 4 had three such races:
    ...
    Every write is now a one-call step gated on the name (X154), and the one read on the close path, `get_events`, never raises. What remains is a *hostile* tool that frees styxx's id and re-takes it under styxx's own name object, which it can read with `get_tool`. The identity test cannot tell that tool from styxx (L-MONITOR). No library does this.
...
- **Audit hooks and slow owners.** An audit hook that blocks on `__code__` writes holds the mutex, and other threads' transactions raise MACHINERY_BUSY after 10 s. If what it blocks on is a lock held by a thread that is itself waiting for the mutex, the two form a cycle. The waiter's bound breaks it after 10 s (I5; `rev5/m5_modelcheck.py`, `with L: exit(X) || exit(Y)+[lock]`: a true deadlock with the bound removed, MACHINERY_BUSY with it).
- **Greenlets.** Untested on a real greenlet library, and modelled. A greenlet switch made from inside a transaction (by a finalizer, audit hook or signal handler) makes the mutex owner look dead, so it is stolen and two transactions interleave. In `rev5/m5_modelcheck.py` (`greenlet: [exit(X), rec] || open(X)`) no property is violated when they do, because every reconciliation step is idempotent. Only a transaction is affected. A switch cannot land inside a one-call step, so the event path has no greenlet exposure (`greenlet: [close(X), open(X)] || open(Y)`: revision 5 clean; revision 4 has U1 violations and 2 end states whose loss is silent).
...
- **L-MONITOR (new).** Any party can call `sys.monitoring.set_local_events`, `set_events` or `register_callback` on styxx's tool id. Doing so blinds the trace, and calls lost before exit are not counted. Revision 5 (MF2) states exactly when the loss is noted. A cleared global PY_UNWIND is noted in any of three ways:
  ...
  - the exit's MONITOR_LOST test finds changed local events or callbacks, or the id freed and re-taken or taken by another tool.

  The loss is **silent** only when the party that changed something restores it before any of these reads: it clears PY_UNWIND and sets it again itself, or it clears a mint's local events or replaces a callback and restores it before exit. Revision 4 said "exit detects it". Its close-time test alone was erased by any other section's open (the fifth critic's MF2, `critic5/c7_external_clear_masked.py`).
  ...
  - **An id another tool took is never touched.** styxx registers no callback and changes no event there, so it cannot clobber the new owner. Revision 5 makes this hold against races too. Every write is a one-call step gated on the name, so a tool that frees and re-takes the id between two of styxx's statements is never written to (the fifth critic's N1; X154). The exception is a hostile tool that re-takes the id under styxx's own name object, read with `get_tool`. styxx cannot tell it from itself. The cost falls the other way: events styxx had set on the freed id, including the global PY_UNWIND if a section was open at the free, keep calling styxx's callbacks there until the new owner changes them (p3: 1,000 callbacks for 1,000 unrelated raises after the exit). This is the one leak after a completed exit, and it is disclosed in the R2-B3 row.
...
- **Scope of testing.** CPython 3.12.3 and 3.13.12, GIL builds. Not covered: other patch levels, 3.14, free-threading, greenlet and gevent on a real library, subinterpreters, and interpreter shutdown. Greenlet switches are covered by the model check only (`rev5/m5_modelcheck.py`). A switch made from inside a transaction defeats the mutex's liveness test, so two transactions can interleave, and the model finds no property violated when they do. A switch cannot land inside a one-call step, so the event path has no greenlet exposure. Revision 4's clearer announcement, which a switch also defeated, is gone (revision 5, N5).
...
| X137d | revision 5, MF2; main thread before the self-trace. Tracers P (declaring f, g) and Q (declaring h). Inside P's section G: call f, then `set_events(tool, 0)` on styxx's id, then call g, whose body raises and is caught, then start a thread that runs `Q.run('B', h)` and join it. Then G closes | P: NOT_EXERCISED for g, MONITOR_LOST. Q: PASS `{h:1}`, no MONITOR_LOST. Without `_unwind_on`'s capture, B's `_unwind_on` sets the event again, G's close finds it set, and P has no MONITOR_LOST (`critic5/c7_external_clear_masked.py`; `rev5/w5_witnesses.py`, both versions) | `_unwind_on` notes blinded openings (rev. 5) |
| X137e | revision 5; in a fresh subprocess. Tracer P is entered and runs section A calling f. Then a second tracer Q is entered. The id-3 tool, PY_START on `_v5_faultpoints()['_ensure_tool']` in Q's enter (after Q's reconciliation), frees styxx's id. Q exits, then P exits | P PASS `{f:1}` with MONITOR_LOST; `_v5_state()["tool_ours"]` is True. With `_ensure_tool` re-taking its own freed id without `_reclaim` (without counting it), P has no MONITOR_LOST (`rev5/w5_witnesses.py`, both versions; `rev5/m5_modelcheck.py` `ext: enter(X) || open(Y)` finds the silent loss this allows) | `_ensure_tool` reclaims its own id (rev. 5) |
...
- **G_HYG (static).** These must hold:
  ...
  - (revision 5) the one-call steps. `_unwind_on`, `_unwind_off`, `_take`, `_register` and `_set_local` each consist of reads of `_TOOL[0]` and `_MON[0]`, then one statement whose outermost call is `_CONSUME(...)` or `list(...)`. Its argument is built only from the M1 one-call names, `_MON[0]`'s functions, `_ANCHORS`, its bound `get` and `pop`, the local `t`, `o`, `o.frame`, `key`, `code` and `events`, and literal tuples. No lambda, generator, comprehension or Python-level function appears in it (the pipelines run Python code nowhere). `_Opening` and `_Core` define no `__getattribute__`, `__getattr__`, property or other Python-level descriptor (M1). `set_events`, `set_local_events`, `register_callback` and `use_tool_id` are called nowhere else, except `_forget_in_child` in the single-threaded child. `_retire` has no step 6. `_reconcile` ends with the prune of cores reached from anchors and then `_unwind_off(None)`, in that order. `_CLEARING`, `_await_clearers` and any wait outside `_acquire` do not exist;
