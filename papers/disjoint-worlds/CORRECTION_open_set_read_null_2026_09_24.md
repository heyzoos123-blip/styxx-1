...
correction was attacked by a fact-checker and a skeptic before it was certified; their findings are
repaired here.

## What the prereg froze

G-O2, in full: "A random-orthogonal mapper, matched in shape and fit on shuffled targets, is run
through the identical pipeline. It must score `AUROC(m) <= 0.55`." The text is internally
inconsistent: a random orthogonal matrix is drawn, not fit, so "orthogonal" and "fit on shuffled
targets" cannot both be meant literally. In this corpus the one other use of the phrase, in
`run_causal_crossworld.py`, is a square QR matrix, which that comment also calls a shuffled-map
null.
...
shuffled targets". It is not orthogonal. It also does not go through the identical pipeline: the
...
- **Had the gate passed, the runner would have emitted a `MIXED__` verdict** (lines 211-215), because
  ...
  say. The RESULT forbids quoting the G-O3 values as results, and they are not repeated here; they
  are in the receipt.

## The bar itself

The RESULT reads G-O2's failure as a mechanism: the margin reading "the geometry of the banks". The
bar cannot carry that. With 35 candidates against 35 out-of-vocabulary concepts, an exchangeable
...
rate: G-O2 as written voids an honest run about as often as not.
...
sentence in `RESULT_extraction_ceiling_REFUTED_2026_09_01.md`, that the run is void "because its
