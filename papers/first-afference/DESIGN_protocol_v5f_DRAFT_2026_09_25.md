...
6. **Append.** `o = _Opening(..., loop=_get_running_loop())`, then `core.openings.append(o)`. `_open` returns o. Steps 1–6 are `_open`, called before `_run`'s `try`.
7. **Commit (revision 3: `_commit(o)`, the first statement of `_run`'s and `_run_async`'s try body).** `_unwind_on()`, then `_ANCHORS[anchor] = o`, then `_unwind_on()` again (revision 2, M4), then the race re-check (step 8), then `o.armed = True`, last. `_unwind_on()` sets the global PY_UNWIND on styxx's id if the id carries `_TOOL_NAME` and the event is not already set. The second call re-sets a clear that landed between the first call and the commit (M7, "The unwind scope"). Revision 2 ran these in `_open`, before the `try`. A fault after the first set then left the event on with no section open, and a fault after the commit left a dead anchor, both until the trace's exit (the third critic's MF2; `rev3/c7r3_stale_unwind_after_open_fault.py`). Inside the `try`, any fault in them is followed by the `finally`'s `_detach`, which pops the anchor if it was committed and clears the event if no other section is open.
8. **Race re-check (inside `_commit`).** If `'exiting'` is now in `core.marks`, `_detach(o, ('open', (OPEN_AT_EXIT,)))` and refuse **TRACE_INACTIVE**. The `finally`'s `_detach` then loses the claim and repeats only idempotent cleanups.
...
- **Close.** In `_run`'s `finally`, on return and on raise: `_detach(o, (end, ()))`.
  ...
  - Before the pop, if `o.armed` is set, the opening's anchor is still registered (`_ANCHORS.get(fr) is o`), the id carries `_TOOL_NAME` and the global events lack PY_UNWIND, `_detach` sets `o.core.flags['UNWIND_LOST']`. Some party cleared the event while the section was open, so exit notes MONITOR_LOST (X137c). Revision 3 adds the first two conditions. Without `o.armed`, a fault inside `_commit` after a concurrent clear set a false flag. Without the anchor test, exit's X3, re-detaching an opening whose close was cut short after its pop, found the event legitimately off and set a false flag; revision 2's inert-opening case did the same (`rev3/c7r3_stale_unwind_after_open_fault.py`: revision 2 notes MONITOR_LOST after a fault right after the append; witnesses X143b and X143c).
...
- **`_Opening`**: `core, section, frame, tid, loop, calls{}, ambiguous{}, fin{}, lazy, armed`. `loop` is the running asyncio loop at open, or None. `armed` is False until `_commit`'s last statement sets it (revision 3).
...
def _commit(o):                                 # revision 3 (At open, steps 7-8)
    _unwind_on()
    _ANCHORS[o.frame] = o
    _unwind_on()                                # re-sets a clear that landed before the commit
    if 'exiting' in o.core.marks:
...
def _detach(o, fin):
    ...
    fr = o.frame
    if fr is not None:
        if o.armed and _ANCHORS.get(fr) is o and _ours() and not (get_events(_TOOL[0]) & PY_UNWIND):
            o.core.flags['UNWIND_LOST'] = True  # revision 3: armed, and still registered
        _ANCHORS.pop(fr, None)
    o.frame = None
    _unwind_off()                               # revision 2

def _unwind_on():                               # _commit: before the anchor commit, and again after it
...
def _unwind_off():                              # after an anchor pop, at _retire step 6, and last in every reconciliation
    if not _ours() or not (get_events(_TOOL[0]) & PY_UNWIND): return    # every call first (revision 3)
    if _ANCHORS or set_events(_TOOL[0], 0): return   # test and clear: one expression, one line (set_events returns None)
    if _ANCHORS: set_events(_TOOL[0], PY_UNWIND)   # fires only under an instruction-level instrument (see below)
```
`_ours()` is `_TOOL[0] is not None and get_tool(_TOOL[0]) == _TOOL_NAME`. styxx's global event set is only ever PY_UNWIND or nothing. `_unwind_on`, `_unwind_off` and `_forget_in_child` are the only callers of `set_events` (G_HYG).

`_run_async` has the same shape, with `_commit(o)` and then `result = await afn(*args, **kwargs)` as its try body. That `await` compiles to a SEND loop. Its throw-then-return path is taken when a thrown exception is handled by the awaited object, which then returns. On that path, the closing jump after CLEANUP_THROW lies outside the try's exception-table range on 3.12.3 and 3.13.12. On 3.12 it is a JUMP_BACKWARD, which checks the eval breaker, so a signal landing there skips the `finally` (`rev1/p19_run_async_throw_path.py`). On 3.13 it is JUMP_BACKWARD_NO_INTERRUPT, which only an injected fault reaches. No count, note, refusal or termination depends on that `finally`. The opening's anchor frame is dead, so it is on no live chain, and exit's X3 or reconciliation finalises the opening as `('open', OPEN_AT_EXIT)`, inside I6. Until then the dead anchor keeps PY_UNWIND set, which is L-DELIVERY scope only (U4 below). `_run`'s try body, `_commit(o)` and the call, has no loop, and neither has `_commit`. No signal position skipped `_run`'s `finally` (p10).

**The unwind scope (revision 2, M4; restated in revision 3, MF1 and MF2).** The global PY_UNWIND is set only while some anchor is registered in this copy, so only while some section is open anywhere in the process. Revision 1 set it while any mint existed, which included a whole trace, zombies, and a freed id after exit. The protocol has no lock:
- the opener, inside `_run`'s `try`, sets the event before its anchor commit and checks again after it, and its body starts only after that;
- every clear is `_unwind_off()`: the closer after its pop, `_retire` step 6, and the last step of every reconciliation. `_unwind_off()` makes its calls first, then tests `_ANCHORS` and clears in one expression on one line.

*Why the test and the clear cannot be separated.* CPython 3.12 and 3.13 switch threads, run signal handlers and the cyclic gc, and deliver asynchronous exceptions only at eval-breaker checks. Those are at RESUME, at backward jumps, and *after* a call returns, never between a load or test and the call that follows it. In `_ANCHORS or set_events(_TOOL[0], 0)` only loads lie between the test and the call, so no other thread runs there. Being one line, the expression also gives a LINE-event tool or a line-tracing function no place to run between them. `rev3/q1_test_then_call_atomic.py` measures this. One thread toggles a dict between empty and non-empty and can be switched out in either state. Another thread tests it and then calls `len()` on it, with a 1 µs switch interval. Three shapes were tried: `if not D: len(D)`, a return on non-empty followed by the call, and `D or len(D)`. None saw the dict non-empty at the call, in 1.17–1.58M empty tests per shape and version. With one C call between the test and the call, it was seen 195–268 times. Revision 2 tested `_ANCHORS` first and called `get_tool` and `get_events` before the clear, which left a switch point between them. That was the toggle window revision 2 disclosed, and `_retire` step 6 had the same gap with no re-check at all (MF1).

*The invariant, with its windows.* Write S for "styxx's id has PY_UNWIND set" and A for "some anchor is registered".
- **U1. An open section's body never runs with S clear.** This holds on every interleaving, fault-free or not, as long as no party outside the machinery clears S (L-MONITOR, X137c). The body starts after the opener's second `_unwind_on()`. After that, a clear needs an `_unwind_off()` whose test finds `_ANCHORS` empty, and this section's anchor is registered until its own close, or until its tracer's exit detaches it at X3, after the credit stop, when nothing the body runs can count any more. A fault in the machinery never clears S: faults only cut a step sequence short, and the only clear is guarded by the test.
- **U2. A clear S with A set lasts only while an opener is between its anchor commit and its second `_unwind_on()`.** That is on the opener's own thread, before its body starts.
- **U3. A set S with no anchor lasts, fault-free, only while one thread is inside its own window.** The windows are an opener between its first `_unwind_on()` and its commit, and a closer or retirer between its pop (or its decision to retire) and the end of its `_unwind_off()`. Each window ends when that thread runs its next few statements, which can include one GIL hand-off after a C call.
- **U4. After faults**, S set with no live section lasts:
  - after a fault in a close between its pop and the end of its `_unwind_off()`, or in `_unwind_off` itself: until the next close that leaves no anchor, or the next transaction anywhere in the process (whose reconciliation ends with `_unwind_off()`);
  ...
  A fault anywhere in `_commit`, which is revision 2's MF2 case, leaves neither: the `finally` pops the anchor and runs `_unwind_off()`.
- **What is lost.** Nothing, fault-free. Revision 2's toggle window, a closer's clear landing after an opener's commit, cannot happen, and a false `UNWIND_LOST` needs S clear under an armed, registered anchor, which U1 excludes.

*The one exception: an instrument that runs Python code inside `_unwind_off`'s test-and-clear expression.* Only an instruction-level instrument does this: the id-5 INSTRUCTION injector while it is armed on `_unwind_off`'s code, a third-party INSTRUCTION tool on it, or a settrace function with `f_trace_opcodes` set on it. LINE-level tools, coverage.py included, do not. Such a callback has eval-breaker checks, so another thread can commit a section between the test and the clear. That restores revision 2's closer window: the re-check re-sets S on the clearing thread's next statement, and in between the new section can lose a confirmation and its close can set `UNWIND_LOST`. This is the only way MONITOR_LOST enters the fault envelope I6. Hence C4 (G_FI).

*Evidence.*
- `rev3/m1_unwind_modelcheck.py` is an abstract step model with a step boundary after every C call. It explores every interleaving of two whole sections and one retiring `_retire` step 6, with 0, 1 or 2 faults at any step boundary.
  - Revision 3: U1 holds on all 1,223 / 4,300 / 7,216 reachable states. U2 and U3 hold on every fault-free state, and no fault-free end state has a stale S. Faulted end states with a stale S and no dead anchor number 0, 1 and 4, and one `_unwind_off()` clears each. `UNWIND_LOST` is set on none.
  - Revision 2: 46 interleavings run a hit with S clear, fault-free. They are the retire race and the closer's toggle window. It also sets `UNWIND_LOST` in 3 fault-free end states.
  - Revision 3 with the test and the clear split into two steps, as the injector splits them: 54 fault-free hits with S clear, all inside the clearer's re-check window (U2 with the clearer's window added).
- `rev3/s1_unwind_stress.py` runs real threads against `mech3.py` at a 1 µs switch interval, with a forced GIL hand-off with probability 0.3 at every statement boundary of the protocol. Three sectioned threads call a raising target in a loop while a fourth enters and exits other tracers (step 6 and the reconciliation's `_unwind_off()`). The fault variants inject faults at those boundaries with probability 0.02, with and without the points inside `_detach` before its pop, which leave dead anchors. It ran 200 rounds × 3 seeds × 3 modes per version. Revision 3, over 368,024 sectioned raising calls and 97,263 injected faults:
  - 0 of 736,048 in-body samples found S clear;
  - every raising call was counted;
  - no run noted MONITOR_LOST;
  - no quiescent point had a stale S in the fault-free runs (0 of 1,200). In the no-dead-anchor fault runs, 38 of 1,200 did, and one close cleared each;
  - in the full fault runs, dead anchors kept S set at every quiescent point, as U4 allows, and each run ended with `anchors == 0` and `global_events == 0` after its exit.

  Revision 2, fault-free, found S clear in 1,793 of 286,442 samples and lost 997 of 143,221 raising calls, with MONITOR_LOST in 6 of 6 runs. Its faulted runs left dead anchors at every quiescent point (F5 in the Revision 3 table) and noted MONITOR_LOST in 7 of 12 runs.
- `rev3/c5r3_retire_step6_race.py` and `rev3/c7r3_stale_unwind_after_open_fault.py` are the critic's two interleavings, on both versions. Revision 2 gives `{}` with MONITOR_LOST, and 33,552–51,880 lost handlers outside sections after one fault in the open path. Revision 3 gives `{'B': {'t': 1}}`, and 0 lost handlers after a fault at each of `_commit`'s five points.
...
- **No `try … finally`, no `with` statement and no threading lock** exist anywhere in the machinery, with one exception: `_run`/`_run_async`'s single `try … finally` around fn.
  - `_run`'s try body is `_commit(o)` and the call of fn, and contains no loop; `_commit` has none either (revision 3).
...
- **I6: the envelope.** A fault inside the machinery can change a trace's record only within these bounds:
  ...
  - notes ⊆ fault-free notes ∪ {OPEN_AT_EXIT} ∪ {MONITOR_LOST, only when an instrument runs Python code inside `_unwind_off`'s test-then-clear: the id-5 injector armed on its code, or opcode tracing} (revision 3, MF7). Without such an instrument no fault clears the event under a registered, armed anchor (U1), so a fault cannot cause `UNWIND_LOST`;
...
- **CLOSE.**
  ...
  - After the claim, before the pop (revision 3 states it): a dead anchor. It keeps PY_UNWIND set until the trace's X3 or its pruning (U4). Scope only, no count.
...
- **G_FI (crash sweep, frozen).** Required on 3.12 and 3.13, on two threads: every fault point is **clean**. Clean is defined by outcome alone, so no window has to be mapped to implementation offsets. A point is clean iff C1–C8 below hold, and C9 as well in the fork scenario. Revision 2's clauses (a)–(e) are C3, C5 with C7, C8, C8's second reading, and C2.
  ...
  *Vocabulary.*
  - The *scenario* is the frozen program run at a point. It constructs tracers only through `coverage_trace()`, catches nothing, and so stops at the first exception that reaches it. The *faulted tracers* are the tracers it constructed.
  ...
  *The invariants:*
  ...
  - **C4, containment (background variant).** A background tracer runs on another thread. It was entered before the sweep and stays active through it, declares the scenario's target (so it shares the mint), and opens its own sections calling that target, which returns rather than raises, so its counts do not depend on PY_UNWIND. At the end of the sweep its record has exactly one count per call it made, and no problem. Its notes are empty, or MONITOR_LOST only if the injector was armed on `_unwind_off`'s code during the sweep (M7, the instrument exception; I6).
  - **C5, nothing left once the faulted tracers are gone.** After step 6, `_v5_state()` equals `S0` except that `cut` may have grown and `tool` may have been set. In particular:
  ...
  - **C6, nothing else escapes.** No exception other than the injected one leaves the scenario, and no thread the scenario started ends with any other exception.
  ...
  - **C8, the envelope.** Every record read at step 2 is within I6 against its tracer's baseline record, or `record()` refused TRACE_INCOMPLETE or TRACE_ACTIVE. Within I6 means:
    ...
    - notes within the baseline notes plus OPEN_AT_EXIT, plus MONITOR_LOST only as C4 allows;
...
- **G_HYG (static).** These must hold:
  ...
  - no loop in any `try` body (CPython #130279). The one allowed `await` is `_run_async`'s; its try body is `_commit(o)`, that `await`, and assignments (revision 3). `_run`'s try body is `_commit(o)`, the call of fn, and assignments;
  ...
  - (revision 3) `_commit` calls `_unwind_on()` both before and after its anchor store and sets `o.armed` as its last statement; `_open` stores no anchor and calls no `_unwind_on`; `_detach`'s `UNWIND_LOST` test reads `o.armed` and `_ANCHORS.get(fr) is o`;
  - (revision 3) in `_unwind_off`, every call (`_ours()`, `get_events`) precedes the `_ANCHORS` test, and the test and the clear are one expression on one line, `if _ANCHORS or set_events(_TOOL[0], 0): return`, so that no call and no line boundary lies between them (M7). `set_events` is called only in `_unwind_on`, `_unwind_off` and `_forget_in_child`; `_retire` step 6 is `_unwind_off()`, and `_reconcile`'s last statement is `_unwind_off()`;
...
- **G_COVER.** Every executable line of the region runs in at least one of the four passes below. No pragma exempts a line. A line that no pass reaches is dead code and must be deleted. Revision 3 drops revision 2's "or folded onto its guard's line" (N12; see "Line coverage only").
  - **Executable lines.** For each region code object, meaning the `_v5_faultpoints()` code objects read before any tracer exists plus the scoring functions of the SM2 region: `{line for _, _, line in code.co_lines() if line is not None} - {code.co_firstlineno}`.
  - **The measurement (revision 3, MF6).** One frozen LINE-event tool, `linecov_v5f.py`, written by the exam author and hashed in the prereg. It uses sys.monitoring tool id 0, which no exam tool or case uses, with global LINE events, because the self-trace mints some region functions and their minted copies must count. Its callback records `(co_filename, line)` for code whose `co_filename` is `styxx/protocol.py`'s path and returns None there. For all other code it returns `DISABLE`. So it never disables a location in machinery code (the injector rule of N5), and its LINE events cannot split `_unwind_off`'s one-line test-and-clear (M7). A line counts iff this tool sees it in some pass. There is no settrace pass: settrace silences the id-5 injector (F1), so a crash sweep run under it never reaches the lines only a fault reaches.
  - **The four passes.** The tool is enabled at the start of every process each pass creates.
    ...
    Lines that the spec requires and that run only in a subprocess are reached in pass 1's subprocess cases: `_reclaim`'s body (X137 free variant), MONITOR_BUSY's raise (X36), and `_ensure_tool`'s id-3 fallback (X142, X137b).
...
2. **L-DELIVERY as possible program corruption.** A PY_UNWIND callback that raises replaces the in-flight exception, and on 3.12.3 and 3.13.12 its `__context__` is None. The second critic answered round-5 question 2 with a probe: yes, an `except ValueError` flow in undeclared code loses its handler under a signal flood, at hundreds of thousands per 2M iterations. Revision 2 confines this to open sections: 0 outside them (`rev2/p4_unwind_scope.py`). It also freezes that G_SIG does not count it as a wrong result. Round 5 should attack what remains:
   ...
   - the per-section toggle. Revision 3 removes its fault-free window by testing and clearing in one expression (M7). Attack that argument's premise: an eval-breaker check, a gc run, or a callback between the test and the call on some 3.12/3.13 patch level, or a tool that runs Python code there. Also attack the dead-anchor bound (U4): a fault at a close's first instruction keeps the event set until that trace exits;
