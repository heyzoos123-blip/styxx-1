...
# shared = (S, A, CLR, G, L, tool, lostn, cores, mint, ops, used, ghost)
S_, A_, CLR_, G_, L_, TOOL_, LOSTN_, CORES_, MINT_, OPS_, USED_, GHOST_ = range(12)
...
class Cfg:
    def __init__(self, name, threads, ncores, covers, intr=(), intr_on=None, ext=(), faults=(0, 1), I=0, X=0, Wsw=0,
                 facade_dead=(), not_entered=(), slots=None, note=''):
...
class Model:
    ...
    def initial(self):
        ...
        ghost = (frozenset(), frozenset(), frozenset(), frozenset())
        sh = (S, A, frozenset(), None, None, 'ours', 0, tuple(cores), mint, tuple(ops), frozenset(), ghost)
    ...
    def step(self, ths, sh, pk, k):
        ...
        if pc == 'B_HIT':
            c = self.op_core[o]
            blind = (not S) and (ops[o][1], o) in A and cores[c][CRED] and not self.core_dead(ths, sh, c)
            sh2 = sh
            if blind:
        ...
        if pc == 'D_S':                                  # the event first (rev 5: a plain read; get_events never raises)
            return upd(pc2='D_A4', r2=setr(r, DS, (not S) and (tool == 'ours' or rev == 5)))
        ...
        if pc == 'X5T':                                  # the MONITOR_LOST test
            c = core
        ...
        if pc == 'E4T':                                  # _ensure_tool
            if tool == 'ours': return upd(pc2='E4M')
            if tool == 'freed':
                if rev == 5 and 'take_split' not in mut:  # _ensure_tool re-takes its own freed id through _reclaim
                    n = sh[LOSTN_] + (0 if 'ensure_nocount' in mut else 1)
                    return upd(pc2='E4M', sh2=sset(sset(sh, TOOL_, 'ours'), LOSTN_, n))
        ...
        if pc == 'E4U':                                  # revision 4: use_tool_id after the test
        ...
        if pc == 'R0':                                   # step 0: reclaim a freed, unowned id
            if tool == 'freed':
                if rev == 5 and 'take_split' not in mut:
    ...
    def externals(self, ths, sh, pk):
        out = []
        tool = sh[TOOL_]; ext = sh[GHOST_][0]
        for kind in self.ext_kinds:
            if kind in ext: continue                     # each external kind at most once per run
            sh2 = None
            ...
            elif kind == 'free' and tool == 'ours': sh2 = sset(sh, TOOL_, 'freed')
            elif kind == 'take' and tool == 'freed': sh2 = sset(sh, TOOL_, 'other')
            if sh2 is not None: out.append((ths, gset(sh2, 0, ext | {kind}), pk))
        return out
...
KEYS = ('U1', 'A0', 'STALE', 'FALSEFLAG', 'CLOBBER', 'P8', 'HANG', 'LOSTNOTE', 'END', 'REPAIR', 'VALUEERROR',
        'CYC_OPEN', 'CYC_TX', 'BUSY_OPEN', 'BUSY_TX', 'REENT', 'LOST', 'MASKED', 'STALE_rev4')
...
def fmt(r):
    ...
    info = [('%s=%d' % (k, r[k])) for k in ('LOST', 'MASKED', 'CYC_TX', 'BUSY_TX', 'BUSY_OPEN', 'REENT', 'STALE_rev4') if r.get(k)]
...
if __name__ == '__main__':
    ...
    if which == 'main':
        ...
        names = ('c8:', 'c3:', 'with L: exit', 'with L: open', 'close(X)+[open(Y)] || open(Y)', 'greenlet:')
        for cfg in CONFIGS:
        ...
        MUTS = ['off_split', 'popoff_split', 'on_ungated_own', 'on_no_capture', 'on_capture_split', 'on_capture_nogate',
                ...
                'take_split', 'ensure_nocount', 'prune_credit_stop']
        sel = sys.argv[2:]
        if sel: MUTS = sel
        for mu in MUTS:
            found = []
            for cfg in CONFIGS:
                if len(cfg.threads) > 2 and cfg.name.startswith('open(X) || open(X) || exit'): continue
                for b in budgets(cfg, 5):
                    r = explore(cfg, 5, b, frozenset([mu]))
                    f = fmt(r)
