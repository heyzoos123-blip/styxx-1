# Revision 7 witnesses on rev7/mech7.py. EVERY trial runs in its own fresh subprocess of the same interpreter
...
def child(case, mut, arg):
    ...
    if case == 'X158':
        ...
        t = mech.TOOL[0]
        M.register_callback(t, E.PY_START, other_cb)   # outside party replaces styxx's entry callback on styxx's id
        Q.run('B', f)                               # lost
