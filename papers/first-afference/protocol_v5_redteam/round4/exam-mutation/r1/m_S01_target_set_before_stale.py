#!/usr/bin/env python
"""EXAM_HOLE: swapping score()'s step 4 (STALE_TRACE) and step 5 (TARGET_SET) survives the frozen
exam: X109 keeps the target set equal and X110 keeps the gates hash equal, so no case has both. The
common real shape has both: the prereg's gates block was edited to add a target and an old trace is
scored against it. The spec order says STALE_TRACE; the mutant says TARGET_SET (wrong reason code).
...
MUTANT_NAME = "S01_target_set_before_stale"
MUTATION = [('        if tr["gates_sha256"] != self.gates_sha256:\n            raise GateSpecError(\n                f"[V5:STALE_TRACE]', '        if sorted(tr["targets"]) != self.coverage_targets:\n            raise GateSpecError(\n                f"[V5:TARGET_SET] gate {name!r}: the trace\'s target set {sorted(tr[\'targets\'])} differs from the declared set {self.coverage_targets}")\n        if tr["gates_sha256"] != self.gates_sha256:\n            raise GateSpecError(\n                f"[V5:STALE_TRACE]')]
...
HARNESS = r'''mod = module("rt4s01_fx", "def f(): return 1\ndef g(): return 2\n")
old = experiment(G={"exercises": ["rt4s01_fx:f"]})                    # the prereg as it was
new = experiment(G={"exercises": ["rt4s01_fx:f", "rt4s01_fx:g"]})     # edited: a target added
with coverage_trace(old) as cov:
    cov.run("G", lambda: (mod.f(), mod.g()))
emit(gate=gate(new, cov.record(), "G"))                                 # the old trace, scored anew'''


def judge(orig, mut):
    o, m = orig["gate"].get("refused", ""), mut["gate"].get("refused", "")
    ok = o.startswith("[V5:STALE_TRACE]") and m.startswith("[V5:TARGET_SET]")
    return ok, ("a stale trace of an edited gates block refuses STALE_TRACE (spec order) in the original "
                "but TARGET_SET in the mutant (wrong reason code)" if ok else "codes did not differ as expected")
