...
PATCHES = {
    'K4': ('''    return _list(_chain(
        _map(_LOST_APPEND, _filter(None, _map(_is_not,
            ...
            _CALLBACKS5))),
        _map(get_tool, (t,))))''', '''    return _list(_map(register_callback, _chain.from_iterable(_map(_repeat,
...
def load(variant):
    src = open(os.path.join(HERE, 'steps7.py')).read()
    if variant != 'ref':
    ...
    mod = types.ModuleType('steps7_' + variant); mod.__file__ = os.path.join(HERE, 'steps7.py')
...
st = dict(active=False, tid=None, stepcode=None, consumer=None, inside=0, opened=0, closed=0, ps_in=[], ps_out=[],
          au_in=[], cc_out=[], snapfn=None, snap_at_call=None)
HARNESS = set()
...
def measure(fn, consumer, snapfn, *args):
    st.update(active=False, tid=threading.get_ident(), stepcode=fn.__code__, consumer=consumer, inside=0, opened=0,
              closed=0, ps_in=[], ps_out=[], au_in=[], cc_out=[], snapfn=snapfn, snap_at_call=None)
...
def verdict(fn_name, mod, expected, post, n_exch=None, extra_ok=True):
    ...
    bad_cc = [getattr(c, '__qualname__', repr(c)) for c in st['cc_out'] if not (c is st['consumer'] or is_from_iterable(c))]
...
def cases(mod):
    ...
    for who in ('styxx', 'styxx-replaced', 'other', 'unowned', 'split1', 'split2', 'split3', 'split4', 'split5'):
        if who == 'other': prep_other(mod, t)
        ...
        ok = ok and r == want_r
        if who in ('styxx', 'styxx-replaced'): ok = ok and read_cbs(mod, t) == list(mod._CALLBACKS5)
        ...
        res.append(('_register', 'owner=%s' % who, verdict('_register', mod, exp, post, n_ex, ok)))
        give_back(mod, t)
    return res
...
if __name__ == '__main__':
    ...
    ok_all = True
    for variant in ('ref', 'K4', 'K5', 'K6', 'K7', 'K8', 'K9', 'K10'):
        ...
        if variant == 'ref':
            ...
            print(v, 'hookup control %s: %d of %d cases fail -> %s; first: %s' % (variant, r['failed'], r['cases'],
                  'DETECTED' if det else 'NOT DETECTED (void)', r['first_failures'][:1]))
            ok_all &= det
    print(v, 'G_ATOM (revision 7 prototype)', 'PASS' if ok_all else 'FAIL/VOID')
