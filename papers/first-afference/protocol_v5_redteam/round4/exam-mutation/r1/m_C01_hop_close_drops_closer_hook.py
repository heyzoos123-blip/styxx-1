#!/usr/bin/env python
"""EXAM_HOLE: _close removes the profile hook only when the closing thread is the opening
thread (`if tid == o.tid and sys.getprofile() is _hook`). The mutant drops `tid == o.tid`. The frozen
exam cannot see it: V28 (the only THREAD_HOP case) closes on a fresh thread that has no hook. When
the closing thread has its own open section, the mutant removes that thread's hook on the hop-close,
so the rest of that section is blind: a false NOT_EXERCISED (plus a PROFILER_LOST note).
...
MUTANT_NAME = "C01_hop_close_drops_closer_hook"
MUTATION = [('                    if tid == o.tid and sys.getprofile() is _hook:', '                    if sys.getprofile() is _hook:')]
...
HARNESS = r'''class _Yield:
    def __await__(self):
        yield
mod = module("rt4c01_fx", "def f(): return 1\ndef g(): return 2\n")
exp = experiment(A={"exercises": ["rt4c01_fx:f"]}, B={"exercises": ["rt4c01_fx:g"]})
with coverage_trace(exp) as cov:
    async def bjob():
        mod.g()
        await _Yield()
    c = cov.run_async("B", bjob)
    t = threading.Thread(target=c.send, args=(None,))     # B opens on a worker thread, suspends
    t.start(); t.join()

    def abody():
        try:
            c.send(None)                                  # B finishes here: closed on this thread
        except StopIteration:
            pass
        mod.f()                                           # A's own work, after that close
    cov.run("A", abody)
rec = cov.record()
emit(A=gate(exp, rec, "A"), B=gate(exp, rec, "B"), notes={s: [n[:22] for o in v for n in o["notes"]] for s, v in rec["sections"].items()})'''


def judge(orig, mut):
    ok = "pass" in orig["A"] and "pass" in orig["B"] and mut["A"].get("refused", "").startswith("[V5:NOT_EXERCISED]")
    return ok, ("the original credits A's own call after a THREAD_HOP close on A's thread (A PASS); the "
                "mutant removed A's thread's hook at that close, so A refuses NOT_EXERCISED" if ok else "no difference")
