#!/usr/bin/env python
"""EXAM_HOLE (hand-built traces only): BAD_TRACE's opening clause is `o["end"] not in _ENDS or
type(o["end"]) is not str`. The mutant drops the exact-type half. The prereg's mutation-audit row
"each BAD_TRACE and BAD_COUNT clause | X96-X115" does not hold for it: the exam's str-subclass edits
(X97c, X98b, X99c, X104c, X105c, X108*) never touch `end`, so the mutant survives. A str-subclass end
is refused BAD_TRACE by the original and accepted (PASS) by the mutant.
...
MUTANT_NAME = "B01_end_type_clause_dropped"
MUTATION = [('            if o["end"] not in _ENDS or type(o["end"]) is not str:', '            if o["end"] not in _ENDS:')]
...
HARNESS = r'''class S(str):
    pass
mod = module("rt4b01_fx", "def f(): return 1\n")
exp = experiment(G={"exercises": ["rt4b01_fx:f"]})
...
def judge(orig, mut):
    ok = orig["gate"].get("refused", "").startswith("[V5:BAD_TRACE]") and "pass" in mut["gate"]
    return ok, ("a str-subclass `end` is refused BAD_TRACE by the original but accepted by the mutant"
