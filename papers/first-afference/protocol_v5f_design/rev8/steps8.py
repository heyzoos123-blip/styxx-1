# Revision 8 (the eighth critic's M1): steps7.py with _register read through itertools.tee, so every previous callback
# stays referenced (by the tee buffer) until the consuming call has made every count; its finalizer, if any, runs only
# when the pipeline is released after the call. _tee joins the one-call vocabulary. Everything else is steps7.py.
...
_tee = itertools.tee                                  # revision 8 (M1): _register's exchanges are read through tee
