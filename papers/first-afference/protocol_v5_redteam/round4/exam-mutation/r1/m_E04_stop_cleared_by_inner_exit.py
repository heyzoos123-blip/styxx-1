#!/usr/bin/env python
"""EXAM_HOLE (fails open): __exit__ clears the dispatch cut _STOP only when the last tracer
exits (`if _ACTIVE == 0: _STOP = None`). The mutant clears it on every exit. The frozen exam cannot
see this because every case's own tracer re-sets _STOP at __enter__, and no case runs cut-dependent
work after a NESTED tracer has exited. With an outer tracer still active after an inner one exits
(V15's shape: nested tracers), a job submitted by another thread into an event loop running inside
cov.run("A") (X65's shape, J1-X2) is credited to A by the mutant: a false PASS. The original refuses
NOT_EXERCISED (the call is dispatched by Handle._run).
...
MUTANT_NAME = "E04_stop_cleared_by_inner_exit"
MUTATION = [('            if _ACTIVE == 0:\n                _STOP = None\n', '            _STOP = None\n            if _ACTIVE == 0:\n')]
...
mod = module("rt4e04_fx", "def f(): return 1\ndef g(): return 2\n")
e_outer = experiment(A={"exercises": ["rt4e04_fx:f"]})
e_inner = experiment(H={"exercises": ["rt4e04_fx:g"]})
with coverage_trace(e_outer) as cov:
    with coverage_trace(e_inner) as inner:         # a nested, shorter trace exits first
        inner.run("H", mod.g)

    def body():                                    # A runs an event loop that serves OTHER threads
        ...
        def foreign():                             # another gate's worker submits ITS job
            ready.wait(5)
            asyncio.run_coroutine_threadsafe(job(), loop).result(5)
            loop.call_soon_threadsafe(mod.f)
    ...
    cov.run("A", body)
rec = cov.record()
emit(gate=gate(e_outer, rec, "A"), calls=rec["sections"]["A"][0]["calls"], uncredited=rec["uncredited"])'''


def judge(orig, mut):
    ok = orig["gate"].get("refused", "").startswith("[V5:NOT_EXERCISED]") and "pass" in mut["gate"]
    return ok, ("a job another thread submitted into the loop inside section A is refused NOT_EXERCISED by "
                "the original (dispatched by Handle._run) but credited to A by the mutant: a false PASS"
