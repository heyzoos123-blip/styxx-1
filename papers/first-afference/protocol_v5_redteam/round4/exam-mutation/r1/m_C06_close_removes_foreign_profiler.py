#!/usr/bin/env python
"""EXAM_HOLE: at the last close on a thread, _close removes the profile function only if it is
the styxx hook (`sys.getprofile() is _hook`). The mutant removes whatever is installed. The frozen
exam cannot see it: no case installs a profiler INSIDE a section and checks it after the close (X80/
X81 install theirs before the open, where the open refuses). The spec: "The foreign profiler is never
called, chained, restored or replaced." Under the mutant a harness's own profiler, started inside a
section, is silently uninstalled when the section closes.
...
MUTANT_NAME = "C06_close_removes_foreign_profiler"
MUTATION = [('                    if tid == o.tid and sys.getprofile() is _hook:', '                    if tid == o.tid:')]
...
HARNESS = r'''mod = module("rt4c06_fx", "def f(): return 1\n")
exp = experiment(G={"exercises": ["rt4c06_fx:f"]})
seen = []
...
with coverage_trace(exp) as cov:
    ...
    sys.setprofile(None)
emit(profiler_still_installed_after_close=still, gate=gate(exp, cov.record(), "G"))'''


def judge(orig, mut):
    ok = orig["profiler_still_installed_after_close"] is True and mut["profiler_still_installed_after_close"] is False
    return ok, ("the harness's own profiler survives the section close under the original but is "
