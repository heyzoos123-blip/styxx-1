# Revision 8 (rev8/m8_modelcheck.py): rev7/m7_modelcheck.py copied, with one mutant added (the eighth critic's M1):
#   'count_after_release'  revision 7's CPython registration: the previous callback is dropped between the exchange and
#                 its count, so an outside party's replaced callback ('F') can run a finalizer there. Modelled as a
#                 step boundary between exchange i and its count (and gate i+1 or the owner read, which follow the count
#                 in C): faults, re-entry and greenlet switches may land there. Revision 8's tee registration keeps the
#                 previous callback alive until every count is made, so the model's rev=7 step (exchange and count in
#                 one step) is revision 8's registration; that merge is an axiom of the model (M7, U1), checked on
#                 CPython by G_ATOM's finalizer case and by X158d.
# Everything else is rev7/m7_modelcheck.py, unchanged.
#
...
class Model:
    ...
    def step(self, ths, sh, pk, k):
        ...
        if pc in ('RGX1', 'RGX2'):                       # exchange i, then (revision 6) gate i+1 or the owner read
            ...
            defer = self.rev >= 7 and cb[i] == 'F' and 'count_after_release' in mut   # revision 8 probe (M1)
            ...
            if defer: return upd(pc2='RGC1' if i == 0 else 'RGC2', r2=r2, sh2=sh2)   # the finalizer's step boundary
        ...
        if pc in ('RGC1', 'RGC2'):                       # revision 8 probe: the count after the finalizer, then gate i+1
            i = 0 if pc == 'RGC1' else 1
            sh2 = sset(sh, LOSTN_, sh[LOSTN_] + 1)
            if i == 1 or (not self.reg5() and tool != 'ours'):
                return ret_(r2=setr(r, RGOWN, tool if not self.reg5() else None), sh2=sh2)
            return upd(pc2='RGX2', r2=r, sh2=sh2)
...
CONFIGS7 = [   # revision 8 adds one cb2+greenlet configuration; revision 7 (M5): outside callback/local-event changes with two cores; registrations with a second live core
    ...
    Cfg('cb2+greenlet: [exit(X), exit(Y)] || open(Y) [cbrep]', [[P('exit', X), P('exit', Y)], [P('open', Y, 0)]], 2,
        'revision 8 (M1): a switch inside X5 hands the mutex to the exit of the trace that lost the call (critic8/c1)',
        Wsw=2, ext=('cbrep', 'cbrep0'), X=1, faults=(0,)),
...
if __name__ == '__main__':
    ...
    if which == 'main':
        ...
        MUTS = ['off_split', 'popoff_split', 'on_ungated_own', 'on_no_capture', 'on_capture_split', 'on_capture_nogate',
                ...
                'retake_nocount+blind_read_gated', 'count_after_release']
        sel = sys.argv[2:]
        if sel: MUTS = sel
