...
- **A new mint.** Otherwise, in this exact order:
  ...
  4. `set_local_events(tool, m.code, PY_START|PY_RESUME|PY_RETURN|PY_YIELD)`. The global PY_UNWIND is not set here: revision 2 sets it only while a section is open (`_commit`, M7).
...
6. **Append.** `o = _Opening(..., loop=loop)`, then `core.openings.append(o)`. `_open` returns o. Steps 1–6 are `_open`, called before `_run`'s `try`. `_open` reads the running loop once, `loop = _get_running_loop()`, at step 5, after steps 1–4 (revision 4: X146b places its hold at that call). `o.frame` is the `_run` frame; only the owning thread's `finally` releases it (step 9).
7. **Commit (`_commit(o)`, the first statement of `_run`'s and `_run_async`'s try body; revision 4 order).** `_ANCHORS[o.frame] = o`; then the race re-check (step 8); then, if `_CLEARING` is non-empty, `_await_clearers()` (M7: wait until every clearer announced at that moment on another thread has withdrawn or died); then one `_unwind_on()`; then `o.armed = True`, last. `_unwind_on()` sets the global PY_UNWIND on styxx's id if the id carries `_TOOL_NAME` and the event is not already set. Because it runs after the store and the wait, no clear can land after it while this anchor is registered, whatever runs between two machinery bytecodes (M7, "The unwind scope"). Revision 3 set the event before the store and again after it, and relied on `_unwind_off`'s test and clear being atomic; revision 4 needs neither (the fourth critic's MF1). Revision 2 ran these steps in `_open`, before the `try`; a fault there left the event on, or a dead anchor, until the trace's exit (the third critic's MF2; `rev3/c7r3_stale_unwind_after_open_fault.py`). Inside the `try`, any fault in them is followed by the `finally`'s `_detach`, which pops the anchor if it was stored and clears the event if no other section is open.
...
- **Close.** In `_run`'s `finally`, on return and on raise: `_detach(o, (end, ()))`, then `o.frame = None` (revision 4: the owner alone releases the anchor frame; At open, step 9).
  - `end` is `"returned"` or `"raised"`.
  - `_detach` claims once with `o.fin.setdefault('fin', value)`, then pops the anchor by `o.frame`, then calls `_unwind_off()` (revision 2). Every step is idempotent. Revision 4: `_detach` no longer assigns `o.frame`, so every detacher (the owner, the race re-check, exit's X3, a prune, the anchor sweep) pops by the anchor's own key.
  - Before the pop, if `o.armed` is set, the id carries `_TOOL_NAME`, the global events lack PY_UNWIND, and the opening's anchor is still registered (`_ANCHORS.get(fr) is o`), tested in that order, `_detach` sets `o.core.flags['UNWIND_LOST']`. Some party cleared the event while the section was open, so exit notes MONITOR_LOST (X137c). Revision 3 added the `o.armed` and anchor conditions. Without `o.armed`, a fault inside `_commit` after a concurrent clear set a false flag. Without the anchor test, exit's X3, re-detaching an opening whose close was cut short after its pop, found the event legitimately off and set a false flag; revision 2's inert-opening case did the same (`rev3/c7r3_stale_unwind_after_open_fault.py`; witnesses X143b and X143c). Revision 4 (MF2) moves the anchor test last. Revision 3 tested the anchor before its two C calls: a close racing its own trace's exit could pass the anchor test, lose the GIL, and read the event that X3's pop and `_unwind_off()` had legitimately cleared, a false MONITOR_LOST with no fault (`rev4/a4r_detach_order.py`, both versions; `rev4/b1_open_races_exit.py`: 23 (3.12.3) and 17 (3.13.12) of 20,000 natural races under revision 3, 38 and 40 under revision 2, 0 under revision 4). Read the other way round, an event found clear while the anchor is still registered afterwards was clear under an armed, registered anchor, which U1 excludes unless a party outside the machinery cleared it (witness X148).
...
_CLEARING = {}  # _Txn -> True: an _unwind_off() between its announcement and its withdrawal (revision 4, MF1)
...
- **Frames the machinery may retain** belong to module-level functions whose own locals hold the core, never the facade: `_run`'s frame, `_run_async`'s coroutine frame, and target frames in `m.pend`. A retained frame that has died still keeps its callers' frames alive, and so possibly the facade; see L-ZOMBIE and L-MONITOR. Revision 4: an opening keeps its anchor frame in `o.frame` until its owner's `finally` releases it. If that `finally` never finishes (a fault in it, or `_run_async`'s `finally` skipped by #130279), the dead frame stays referenced by the opening until the core is dropped. Revision 3's X3 released it at exit; revision 4 cannot, because a release by any thread other than the owner is what made B1 possible.
- **`_Opening`**: `core, section, frame, tid, loop, calls{}, ambiguous{}, fin{}, lazy, armed`. `loop` is the running asyncio loop at open, or None. `armed` is False until `_commit`'s last statement sets it (revision 3). `frame` is assigned by `_open` and, to None, only by its owner's `finally` (revision 4).
- **`_Txn(tid, fid, code, succ={})`** is created by `me = _txn()` from the caller's frame. It is *live* iff some frame f on `sys._current_frames()[tid]`'s `f_back` chain has all three of: `id(f) == fid`, `f.f_code is code`, and `f.f_locals.get('me') is` the token. The check is exact, holds no frame, and needs no callback. Revision 4 uses the same token and the same liveness test (`_alive`) for a clearer's announcement in `_CLEARING`: `_unwind_off` creates `me = _txn()`, so the token is live exactly while that `_unwind_off` call is executing.
...
2. If `_named(tool)`: `set_local_events(tool, m.code, 0)`. An id that no longer carries this copy's name is never touched (L-MONITOR).
3. `m.pend.clear()`.
4. Pop `_BY_FN[m.fn]` if its value is m.
5. Pop `_MINTED[id(m.code)]` if its value is m.
6. `_unwind_off()` (M7). Revision 2's step 6 tested `_ANCHORS`, then called `get_tool`, then cleared. Openers do not take the mutex, so a section could open between the test and the clear and then run its whole life without PY_UNWIND (the third critic's MF1; `rev3/c5r3_retire_step6_race.py`: revision 2 `{}` with MONITOR_LOST, revision 3 `{'B': {'t': 1}}`). `_unwind_off()` makes its calls first and tests `_ANCHORS` immediately before the clear, so no open can fall between them.

**Anchor sweep (revision 4, B1).** After the holders are pruned: for each `(fr, o)` in a `list(_ANCHORS.items())` snapshot whose `o.core` is dead by the holder test above, `_prune(o.core)` (the credit stop `by_code = {}`, then `_detach` of each of its openings), then `_ANCHORS.pop(fr, None)`. The credit stop comes first for the same reason as in `_prune`: a core can die between the prune loop and the sweep, when its exit is interrupted between X1 and X2, and the model check found that a sweep which only detached then popped the anchor of a still-crediting section, whose next raising call was then lost (`rev4/m1_pairs_modelcheck.py`, mutant `sweep_no_credit_stop`: U1 violated in 31 faulted paths of open ∥ exit ∥ reconcile; 0 with the credit stop). The prune above reaches only cores that still hold a registered mint. This sweep also reaches an anchor whose core has already exited or released its mints: after two faults in an open that raced its trace's exit (a fault after `_commit`'s store, then one before the `finally`'s pop; X146d). A key maps to one opening only (it is that opening's `_run` frame), so the unguarded pop cannot remove another opening's anchor.

**Announcement sweep (revision 4, MF1).** Every token in a `list(_CLEARING)` snapshot that `_alive` finds dead is popped. Such a token belongs to a clearer that a fault ended inside its window (X149b). Openers skip dead tokens anyway (`_await_clearers`), so this only keeps `_v5_state()["clearing"]` at 0 at quiescence.

**Last step of every reconciliation (revision 3, MF2).** `_unwind_off()`, whether or not a mint retired. Together with the close's own call, this bounds a set event left with no anchor by a fault: it lasts until the next close that leaves no anchor, or the next transaction anywhere in the process (M7, "The unwind scope").
...
- **E4. `_locked(_enter_txn, core, resolved)`,** which runs:
  1. reconcile;
  2. `_ensure_tool()`:
     - If `_TOOL[0]` is set and `_named(_TOOL[0])`, keep it. `_named(i)` is `n = get_tool(i); return type(n) is str and n == _TOOL_NAME` (revision 4, N1): `use_tool_id` accepts a str subclass as another tool's name, and a plain `get_tool(i) == _TOOL_NAME` would run that subclass's `__eq__`, which can return True and make styxx adopt a foreign id (`critic4/a5_tool_name_eq.py`, both versions; X142b).
...
The open checks are listed under Attribution. The rest (revision 4):
```python
def _run(core, section, fn, args, kwargs):      # module level: its frame is the anchor, its locals hold no facade
    ...
    try:
        ...
        o.frame = None                          # revision 4 (B1): the owner alone releases the anchor frame
...
def _commit(o):                                 # At open, steps 7-8 (revision 4 order)
    _ANCHORS[o.frame] = o                       # o.frame is set: only this thread's finally releases it
    ...
    if _CLEARING: _await_clearers(o)            # clearers that may have tested _ANCHORS before the store
    _unwind_on()                                # after the store and the wait: no clear can follow while o is registered
    o.armed = True                              # last: the body may run now

def _await_clearers(o):                         # revision 4 (MF1); no try: a fault here propagates into _run's try
    me = threading.get_ident()
    w = [t for t in list(_CLEARING) if t.tid != me]   # announced now, on other threads (a list comprehension)
    t0 = time.monotonic(); spins = 0
    while w:
        for t in list(_CLEARING):
            if not _alive(t): _CLEARING.pop(t, None)  # a clearer that a fault ended inside its window
        w = [t for t in w if _CLEARING.get(t) is not None]
        if w and time.monotonic() - t0 > _BUSY_SECONDS:     # a live clearer held for 10 s (I5)
            o.core.problems.append(MACHINERY_BUSY_TEXT)
            raise GateSpecError(MACHINERY_BUSY_TEXT)
        spins += 1
        if w: time.sleep(0 if spins <= 100 else 0.0002)     # yield the GIL first, then back off as _acquire does

def _detach(o, fin):
    o.fin.setdefault("fin", fin)                # one claim: first finaliser wins
    fr = o.frame                                # revision 4: never assigned here, so every detacher pops by the key
    if fr is not None:
        if o.armed and _ours() and not (get_events(_TOOL[0]) & PY_UNWIND) and _ANCHORS.get(fr) is o:
            o.core.flags['UNWIND_LOST'] = True  # revision 4 (MF2): the event first, the anchor last
        _ANCHORS.pop(fr, None)
    _unwind_off()                               # revision 2

def _unwind_on():                               # _commit only: after the store and the wait
    if _ours() and not (get_events(_TOOL[0]) & PY_UNWIND): set_events(_TOOL[0], PY_UNWIND)

def _unwind_off():                              # after an anchor pop, at _retire step 6, and last in every reconciliation
    if not _ours() or not (get_events(_TOOL[0]) & PY_UNWIND): return
    me = _txn()                                 # revision 4 (MF1): this call's token, live while this frame runs
    _CLEARING[me] = True                        # announce, before the test
    if not _ANCHORS: set_events(_TOOL[0], 0)    # the test, then the clear: they need not be atomic
    _CLEARING.pop(me, None)                     # withdraw, after the clear

def _named(i):                                  # revision 4 (N1): the exact type first, so no foreign __eq__ runs
    n = get_tool(i)
    return type(n) is str and n == _TOOL_NAME

def _ours(): return _TOOL[0] is not None and _named(_TOOL[0])
```
styxx's global event set is only ever PY_UNWIND or nothing. `_unwind_on`, `_unwind_off` and `_forget_in_child` are the only callers of `set_events` (G_HYG). Every comparison with `_TOOL_NAME` goes through `_named` (G_HYG).

`_run_async` has the same shape, with `_commit(o)` and then `result = await afn(*args, **kwargs)` as its try body, and the same two statements in its `finally`. That `await` compiles to a SEND loop. Its throw-then-return path is taken when a thrown exception is handled by the awaited object, which then returns. On that path, the closing jump after CLEANUP_THROW lies outside the try's exception-table range on 3.12.3 and 3.13.12. On 3.12 it is a JUMP_BACKWARD, which checks the eval breaker, so a signal landing there skips the `finally` (`rev1/p19_run_async_throw_path.py`). On 3.13 it is JUMP_BACKWARD_NO_INTERRUPT, which only an injected fault reaches. No count, note, refusal or termination depends on that `finally`. The opening's anchor frame is dead, so it is on no live chain, and exit's X3, a prune, or the anchor sweep finalises the opening as `('open', OPEN_AT_EXIT)`, inside I6. Until then the dead anchor keeps PY_UNWIND set, which is L-DELIVERY scope only (U4 below), and the opening keeps its dead anchor frame (M1). `_run`'s try body, `_commit(o)` and the call, has no loop, and neither has `_commit`. `_await_clearers` has a loop and no `try`. It runs only while `_CLEARING` is non-empty, and an exception at its back-edge, #130279's shape, propagates out of its own frame into `_run`'s `try`, whose `finally` runs (`rev4/h4_callee_backedge.py`: the finally ran on 3.12.3 and 3.13.12, while the control, the same loop written inside the `try` body, skipped it on both). No signal position skipped `_run`'s `finally` (p10).

**The unwind scope (revision 2, M4; restated in revision 3; made independent of atomicity in revision 4, MF1).** The global PY_UNWIND is set only while some anchor is registered in this copy, so only while some section is open anywhere in the process. Revision 1 set it while any mint existed, which included a whole trace, zombies, and a freed id after exit. The protocol has no lock:
- the opener, inside `_run`'s `try`, stores its anchor, re-tests its claim (step 8), waits for every clearer announced at that moment on another thread to withdraw or die, and only then sets the event and arms;
- every clear is `_unwind_off()`: the closer after its pop, `_retire` step 6, and the last step of every reconciliation. `_unwind_off()` announces itself in `_CLEARING` before it tests `_ANCHORS`, and withdraws after its clear.

*Why no clear can land under an armed anchor.* Take an opener O and a clearer C whose clear lands while O's anchor is registered and O is armed. C's test found `_ANCHORS` empty, so C tested before O's store. C announced before its test, so C was announced before O's store, and so before O read `_CLEARING` after the store. Then O waited until C withdrew or died. C withdraws after its clear, and a dead C clears nothing more, so C's clear preceded O's `_unwind_on()`, which then found the event clear and set it. So C's clear did not land while O was armed: a contradiction. The argument uses only the order of single C operations on one thread (a dict store, a dict read, one `sys.monitoring` call), which the GIL makes atomic. It does not use the absence of an eval-breaker check between a test and a call, so it holds whatever Python code runs between two machinery bytecodes: another thread switched in, a signal handler, the cyclic gc, or an instrument's callback. The one exclusion is C on O's own thread: O skips tokens of its own thread, because such a C is suspended below O on the same stack and waiting for it would deadlock. Such a C cannot clear while O's body runs on this thread, only after O's section has closed, with one contrived exception, R19. Revision 3 relied instead on `_ANCHORS or set_events(_TOOL[0], 0)` having no eval-breaker check between the test and the call. That holds against thread switches, signals, gc and asynchronous exceptions (`rev3/q1_test_then_call_atomic.py`). It fails under any tool that runs Python code at an instruction inside the expression, including tools this spec says coexist (the fourth critic's MF1; `critic4/a1_what_splits_test_and_clear.py`):
- a `sys.setprofile` or `threading.setprofile_all_threads` function (V48's shape), and cProfile with a Python timer;
- a sys.monitoring CALL callback, local or global (a C_RETURN callback with no CALL callback does not split it);
...
LINE events, from sys.monitoring or settrace, and JUMP events did not split it. Nothing in revision 4 depends on this list. `rev4/m2_split_instruments.py` splits `_unwind_off` deterministically with four of these instruments (a pure-Python profiler, opcode tracing, CALL with C_RETURN, INSTRUCTION) while an opener commits between the test and the clear, and A's body raises only after the clear: revision 3 loses the call and notes MONITOR_LOST under all four, on both versions; revision 4 counts it with no note under all four.

*The invariant, with its windows.* Write S for "styxx's id has PY_UNWIND set" and A for "some anchor is registered".
- **U1. An armed section's body never runs with S clear.** This holds on every interleaving of single C operations, fault-free or not, whatever Python code other tools run between machinery bytecodes, with two exclusions. First, no party outside the machinery may clear S (L-MONITOR, X137c). Second, R19. The body starts after the opener's `_unwind_on()`, and by the argument above no `_unwind_off()` clears S while the opener's anchor is registered. The anchor stays registered until its own close, or until its core stops crediting and a detacher pops it: exit's X3 after the credit stop, or a reconciliation's prune or anchor sweep of a dead core (revision 4, N2). A prune sets `h.by_code = {}` before it detaches. After either, nothing the body runs can count. A fault in the machinery never clears S: faults only cut a step sequence short, and the only clear is guarded by the test.
- **U2. A clear S with A set lasts only while an opener is between its anchor store and its `_unwind_on()`,** including its wait. That is on the opener's own thread, before its body starts.
- **U3. A set S with no anchor lasts, fault-free, only while one thread is inside its own window.** A detacher's window runs from its pop (or a retirer's decision to retire) to its clear: a closer, exit's X3, a prune, a sweep, or `_retire` step 6. The other window is an opener whose anchor exit's X3 popped after the opener's step 8. That opener's body runs uncredited, with S set by its own `_unwind_on()`, until its own close clears S. Each window ends on that thread's own later statements.
- **U4. After faults**, S set with no live section lasts:
  - after a fault in a detacher between its pop and its clear, or in `_unwind_off` itself: until the next close that leaves no anchor, or the next transaction anywhere in the process (whose reconciliation ends with `_unwind_off()`). A fault inside `_unwind_off` after its announcement also leaves a dead token in `_CLEARING`. Openers skip it (`_alive`), and the next reconciliation pops it (X149, X149b);
  - after a fault in a close before its pop (at `_detach`'s first instructions, or a `_run_async` `finally` skipped by #130279 on 3.12): a *dead anchor* stays registered, so no `_unwind_off()` clears S until the owning trace's exit detaches it at X3, or a reconciliation prunes or sweeps it once its core is dead (revision 4: the anchor sweep also reaches a core that already exited, X146d). This is L-DELIVERY scope only. The dead anchor's frame is on no live chain, so no count, note or refusal depends on it.

  A fault anywhere in `_commit`, which is revision 2's MF2 case, leaves neither: the `finally` pops the anchor (`o.frame` is still set) and runs `_unwind_off()`.
- **What is lost.** Nothing, fault-free, on any interleaving at the granularity of single C operations. That rules out revision 2's toggle window and revision 3's split window. A false `UNWIND_LOST` needs S read clear while the anchor is registered afterwards, under an armed opening. U1 excludes that, and `_detach` reads S before it tests the anchor (MF2).

*Evidence (revision 4).*
- `rev4/m1_pairs_modelcheck.py`: every interleaving, at single-C-operation granularity, of every pair of concurrent transitions (open, close, exit of the same and of another tracer, a second exit, a pruning reconciliation) and four triples, with up to two faults. Revision 4 violates none of U1, A0, HANG, END, REPAIR and P8 in any of 15 configurations (49.5 million states). Revision 3 leaves `_ANCHORS[None]` and sets false flags when exit races its own tracer's sections, and with its test and clear split it violates U1 fault-free. The full enumeration is in the Revision 4 section.
...
2. `_ANCHORS.clear()` and `_CLEARING.clear()` (revision 4: the parent's clearer tokens name threads the child does not have);
...
- **Commits are single C operations:** dict setitem, `setdefault`, `pop` and `get` with str, int, frame, function, `_Core` or `_Txn` keys (identity or str hashing, so no Python `__hash__` or `__eq__` runs); `list.append`; `set.clear`; one STORE_ATTR or STORE_SUBSCR; one `sys.monitoring` call. The one comparison with a foreign object, another tool's name from `get_tool`, tests `type(n) is str` first (`_named`, revision 4, N1), so no foreign `__eq__` runs either. The machinery's correctness arguments use only the order of these operations on each thread, never the absence of an eval-breaker check between two of them (revision 4, MF1).
- **No `try … finally`, no `with` statement and no threading lock** exist anywhere in the machinery, with one exception: `_run`/`_run_async`'s single `try … finally` around fn.
  - `_run`'s try body is `_commit(o)` and the call of fn, and contains no loop; `_commit` has none either (revision 3). Revision 4: `_commit` calls `_await_clearers`, whose wait loop has no `try`. #130279 affects the frame that holds the back-edge, so an exception there leaves `_await_clearers` and `_commit` and reaches `_run` at its call, inside the `try` range, and the `finally` runs (`rev4/h4_callee_backedge.py`). The `finally` itself is `_detach(o, …)` and `o.frame = None`, with no loop.
...
- **I3: reconciled registries.** After any reconciliation:
  ...
  - global PY_UNWIND is clear if no anchor is registered and no thread is inside its own window (M7, U3). Every reconciliation ends with `_unwind_off()`, so right after it the event is set iff an anchor is registered, except for the reconciling thread's own window and the windows of threads running concurrently. An anchor whose section's body may run always has the event set (U1). Revision 2 said "set only while some anchor is registered; a close or reconciliation repairs a stale set", and both halves were false (the third critic's MF1, MF2). Revision 3's version of this clause was false after an open that raced its trace's exit, which left `_ANCHORS[None]` registered for the rest of the process (the fourth critic's B1); revision 4 closes that (At open, steps 8 and 9);
  - (revision 4) no registered anchor belongs to a dead core (the anchor sweep), and no token in `_CLEARING` belongs to a clearer that is no longer running (the announcement sweep);
...
- **I5: no blocking state held by the dead.** The only exclusion is the robust mutex. A dead owner is stolen; a live owner on the same thread gives REENTRANT instead of a wait; a live owner on another thread is waited on for at most 10 s (MACHINERY_BUSY). No threading lock exists, so a skipped release cannot hang anyone. Revision 4 adds one more wait, with the same rules: an opener waits in `_await_clearers` only for clearers on other threads that `_alive` finds running, never for its own thread's, and for at most 10 s, after which it records and raises MACHINERY_BUSY. A clearer never waits for anything, so no cycle of waits exists inside the machinery. A clearer held inside its window by user code (a profiler, a signal handler, a lower-id tool) holds up openers until it proceeds, at most 10 s each (X150, V66). The model check found no reachable state where live threads exist and none can step (HANG) in any pair or triple.
- **I6: the envelope.** A fault inside the machinery can change a trace's record only within these bounds:
  ...
  - notes ⊆ fault-free notes ∪ {OPEN_AT_EXIT}. No fault and no instrument clears the event under a registered, armed anchor (U1), so a fault cannot cause `UNWIND_LOST`. Revision 3 had to allow MONITOR_LOST here whenever an instrument runs Python code inside `_unwind_off`'s test-and-clear, the id-5 injector armed on its code included; revision 4 removes that exception (the fourth critic's MF1, N3, N4);
...
- **OPEN.**
  ...
  - Inside `_commit` (revision 3), at any point, including inside `_await_clearers` and at its loop's back-edge (revision 4): the fault is inside `_run`'s `try`, so the `finally`'s `_detach` pops the anchor if it was stored (by `o.frame`, which only this `finally` releases, after the pop) and runs `_unwind_off()`. The opening ends `"raised"` and fn never runs. `o.armed` is still False, so no `UNWIND_LOST`. A second fault in that `finally` before its pop leaves a dead anchor: until the trace's X3 or, if the trace has already exited (the open raced its exit), until the next reconciliation's anchor sweep (X146d). Revision 2 ran these steps before the `try`. A fault after the first set left the event on with no anchor, and a fault after the commit left a dead anchor. Either kept PY_UNWIND on outside every section until the trace exited, and the next close did not clear it (`rev3/c7r3_stale_unwind_after_open_fault.py`: 33,552–51,880 lost handlers in 300,000 iterations after one fault, against 0).
- **CLOSE.**
  ...
  - After the claim, before the pop (revision 3 states it): a dead anchor. It keeps PY_UNWIND set until the trace's X3 or its pruning or sweep (U4). Scope only, no count.
  - After the pop, before `_unwind_off()` finishes: the event may stay on with no anchor until the next close that leaves no anchor, or the next transaction, whose reconciliation ends with `_unwind_off()` (revision 3, MF2). Scope only, no count. Exit's X3 later re-detaches this opening, and the anchor test in `_detach` keeps that from setting a false `UNWIND_LOST` (X143c). A fault inside `_unwind_off` after its announcement also leaves a dead token in `_CLEARING`: openers skip it and the next reconciliation pops it (X149, X149b).
...
- **Reconcile and retire.** Idempotent. A second fault inside them is repaired by the next transaction. The revision-4 sweeps are idempotent too: each pops only what is dead, and `_detach` of an already-finalised opening repeats only cleanups.
- **Tool acquisition.** The callbacks are registered before `_TOOL[0]` is stored. A tool left named ours with missing callbacks is adopted and re-registered at the next enter.
  - Two races with other tools become faults of this model. No handler catches them, because a handler could swallow an asynchronous ValueError:
    ...
    - A `free_tool_id` on another thread landing between `_ours()` and `get_events` or `set_events` in `_unwind_on`, `_unwind_off` or `_detach` makes that call raise ValueError ("tool N is not in use") out of `run()` or its close.
...
- **Greenlets.** Untested. A greenlet switch made from inside a transition (by a finalizer, audit hook or signal handler) makes the owner look dead, so it is stolen. The same holds for a clearer's announcement (revision 4): a greenlet switch inside `_unwind_off`'s window makes the clearer look dead to openers, and if it is switched back later its clear can land under an armed anchor. That call is lost, and MONITOR_LOST is noted at its close.
- **R19, a same-thread clearer (revision 4; deliberate only).** `_await_clearers` skips announcements of the opener's own thread. Such a clearer is suspended below the opener on the same stack, so waiting for it would deadlock. It cannot clear while the opener's body runs on that thread. It can clear while the section is still open only in one shape. Code that interrupted that `_unwind_off` inside its window opens a section on the same thread: a signal handler, a finalizer run by the gc, or a profiler or lower-id tool callback. The section must be a `run_async` coroutine left suspended when that code returns, and resumed on another thread. The interrupted `_unwind_off` then clears the event while the section's anchor is registered, and the section's raising calls are lost until some other opener sets the event again. Its close notes MONITOR_LOST (the `UNWIND_LOST` test), so the loss is never silent. No known library opens a coroutine section from a signal handler or a profiler callback.
...
| MACHINERY_BUSY | **new** | another thread held the machinery for more than 10 s; or (revision 4) an open waited more than 10 s for a clearer on another thread that stayed inside its `_unwind_off` window | enter: raise. Exit: recorded. Open: raise + recorded |
...
20. **Lifecycle refusals.**
    ...
    - (Revision 4) MACHINERY_BUSY at open when another thread stays inside `_unwind_off`'s window for more than 10 s, held there by user code such as a blocking profiler callback or signal handler (X150). An open never waits for a clearer on its own thread.
21. **The dispatch cut.**
...
- **L-DELIVERY (new; scope reduced in revision 2).** styxx's callbacks are Python code at these events:
  ...
  - while *some section is open* anywhere in the process (revision 2; revision 1: while any mint existed): every exception unwind of any Python frame, on every thread. Revision 3 states the exceptions exactly (M7, U3 and U4). A thread's own window adds a few statements. So does the body of a section whose anchor exit's X3 popped after its step 8, until its own close. After a fault in a close, the event stays until the next close that leaves no anchor or the next transaction. After a fault at a close's first instructions, or a `_run_async` `finally` skipped by #130279, a dead anchor keeps it until that trace's exit, or until a reconciliation prunes or sweeps it once the trace is dead. Revision 3 could also leave it set for the rest of the process after an open raced its own trace's exit (B1); revision 4 closes that.
...
| X143 | revision 2, M4; restated for revision 4; main thread before the self-trace (revision 3, MF3). Thread T2 opens section B and waits inside it. Thread T1 calls `cov.run('A', body)`, where body calls t and t raises inside its body; body catches it. The id-3 tool, with PY_START on `_v5_faultpoints()['_commit']` and filtered to T1, holds T1 at its PY_START of `_commit`, which is before A's anchor store. The case then lets T2 close B, the last registered anchor, so B's `_unwind_off()` clears PY_UNWIND. Then it releases T1 | A PASS `{t:1}`: the opener sets the event itself, after its store and wait. Without the `_unwind_on()` in `_commit`, t's unwind is never seen and A refuses NOT_EXERCISED with MONITOR_LOST (`rev4/w4_rev4_witnesses.py`, both versions; revision 3's form of this case held T1 at its first return from `_unwind_on`, which revision 4 no longer calls before the store: `rev2/p14_recheck_witness.py`, `rev3/w1_rev3_witnesses.py`) | M4 (rev. 2); `_commit`'s `_unwind_on` (rev. 4) |
| X143b | revision 3, MF1/MF7; main thread before the self-trace. X143's setup. After T2's close has cleared the event, the id-3 tool raises `Injected` at PY_START of T1's `_unwind_on`, which is after A's anchor store (and wait) and before its set | `Injected` propagates from T1's `run()`; the body never ran; A's end is `"raised"`; the trace's notes contain no MONITOR_LOST; after exit `anchors == 0` and `global_events == 0`. Without `o.armed` in `_detach`'s check, the close sees its registered anchor with the event clear and notes MONITOR_LOST (`rev3/w1_rev3_witnesses.py`; `rev4/w4_rev4_witnesses.py`, both versions) | `o.armed` guard (rev. 3) |
| X143c | revision 3, MF2; main thread before the self-trace. `cov.run('A', f)`, with the id-5 injector armed on `_v5_faultpoints()['_detach']`, raising `Injected` at the first instruction at which `_v5_state()["anchors"]` is 0, which is after the pop and before `_unwind_off()`. Then a second tracer declaring f is entered and exited with no section, while the first is still active, so its exit retires no mint. Then the first tracer exits | `Injected` from `run()`. Right after it, `global_events` is PY_UNWIND (U4's window). After the second tracer's transaction it is 0. The first trace PASS `{f:1}`, A's end `"returned"`, no MONITOR_LOST. Without the anchor test in `_detach`, exit's X3 re-detaches A and notes MONITOR_LOST. Without `_unwind_off()` at the end of reconciliation, `global_events` stays PY_UNWIND after the transaction (`rev3/w1_rev3_witnesses.py`; `rev4/w4_rev4_witnesses.py`, both versions) | anchor test in `_detach`; reconciliation's last step (rev. 3) |
| X144 | revision 3, MF2; main thread before the self-trace, no other section open. The id-3 tool, with a local PY_RETURN event on `_v5_faultpoints()['_unwind_on']`, raises `Injected` at the case thread's *first* return from `_unwind_on` in `cov.run('A', f)` | `Injected` propagates from `run()`, and f was not called. Immediately afterwards, before any other open, close or transaction, `_v5_state()` shows `anchors == 0` and `global_events == 0`. After exit: A NOT_EXERCISED, end `"raised"`, no MONITOR_LOST; a later section PASS `{f:1}`. With `_commit` before the `try` (revision 2's placement), `global_events` is PY_UNWIND with no anchor until the trace exits (`rev3/w1_rev3_witnesses.py`; `rev3/c7r3_stale_unwind_after_open_fault.py`; `rev4/w4_rev4_witnesses.py`). Revision 4's only `_unwind_on` call is after the store and the wait, so the fault lands after the set | `_commit` inside the try (rev. 3) |
| X145 | revision 3, MF1; rewritten in revision 4 (MF1, MF5); main thread before the self-trace. (a) Closer: T2 closes section B, the last anchor. The id-3 tool registers, on `_v5_faultpoints()['_unwind_off']`, a local CALL event and a PY_RETURN event, and PY_START on `_v5_faultpoints()['_await_clearers']`, all filtered by live thread ident. At T2's CALL whose callable is `sys.monitoring.set_events` (the clear, after the test found `_ANCHORS` empty), it holds T2 until Event `release` is set. The case then starts T1: `cov.run('A', body)`. At T1's PY_START of `_await_clearers`, the tool sets `release`. At T2's PY_RETURN from `_unwind_off`, after the hold, it sets `t2_done`. body sets `release`, waits for `t2_done`, then calls t, which raises inside its body and is caught. No step waits on a timeout: under the spec T1 reaches `_await_clearers` and releases T2, which clears, withdraws and returns before A's body starts; under each mutant A's body starts first and releases T2, whose clear then lands before t raises. (b) Retire: tracer Y is active with no section open. Thread X exits tracer Z, whose target mint retires. The tool, PY_START on `_ours` filtered to X, holds X at its first `_ours` call after Z's target has left `_v5_state()["mints"]`, until section B of Y is open on another thread; B's body calls t after X has finished | (a) A PASS `{t:1}`, no MONITOR_LOST. (b) B PASS `{t:1}`, no MONITOR_LOST. Each of these mutants gives (a) A NOT_EXERCISED and MONITOR_LOST: `_unwind_off` without its announcement; the announcement withdrawn before the test; `_commit` without `_await_clearers`; `_unwind_on` before the wait. `_retire` step 6 as revision 2's test-then-clear gives (b) B NOT_EXERCISED and MONITOR_LOST (`rev4/w4_rev4_witnesses.py`, both versions). The tool's CALL callback runs Python code inside `_unwind_off`; that is what the case needs, and revision 4 tolerates it (M7) | the clearer's announcement and the opener's wait (rev. 4); step 6 is `_unwind_off()` (rev. 3) |
| X146 | revision 4, B1; main thread before the self-trace. T1 calls `cov.run('A', f)`. The id-3 tool, PY_START on `_v5_faultpoints()['_commit']` filtered to T1, holds T1 there until the main thread's `cov.__exit__()` has returned (X3 claimed A while A had no anchor). Then T1 is released and joined | T1's `run()` raises TRACE_INACTIVE. After the join `anchors == 0`, `global_events == 0`, `clearing == 0`; `record()` has A's end `"open"` with OPEN_AT_EXIT and the problem TRACE_INACTIVE; score refuses TRACE_INACTIVE. With `_detach` assigning `o.frame = None` (revision 3), `anchors == 1` after the join (`_ANCHORS[None]`). With step 8's re-check deleted, `run()` returns f's value (`rev4/w4_rev4_witnesses.py`, both versions; the fourth critic's a3: revision 3 leaves `anchors 1`, key NoneType, `global_events 4096`, also after a later clean trace) | owner-only release of `o.frame`; step 8 (rev. 4) |
...
| X146c | revision 4, B1; main thread before the self-trace. T1 creates `co = cov.run_async('A', af)` and drives it with `asyncio.run`. The id-3 tool holds T1 at PY_START of `_commit`. The main thread drops its last reference to the facade (`del cov`; `gc.collect()`), then enters and exits a second tracer declaring g, whose reconciliation prunes A's dead core (claiming A; `'exiting'` is never set). Then T1 is released | `co` raises TRACE_INACTIVE; `anchors == 0`, `global_events == 0`. With step 8 testing only `'exiting'`, `co` returns af's value | step 8's claim test (rev. 4) |
| X146d | revision 4, B1; main thread before the self-trace. X146's hold, and then two injected faults on T1: the id-5 injector raises `Injected` at the first instruction of `_commit` at which `_v5_state()["anchors"]` is 1 (after the store), and again at the first instruction of `_detach` in the `finally` (before its pop). Then a second tracer declaring g is entered and exited | `Injected` from T1's `run()`; right after it `anchors == 1` (a dead anchor of an exited core); after the second tracer's transaction `anchors == 0` and `global_events == 0`. Without the reconciliation's anchor sweep, `anchors` stays 1 | the anchor sweep (rev. 4) |
| X147 | revision 4, N5; main thread before the self-trace; `envelope` class (timing). 200 trials. Each enters a tracer declaring f, starts T1 running `cov.run('A', f)`, calls `cov.__exit__()` on the main thread without waiting, joins T1, then reads `record()`. f returns | Every trial is one of: (i) T1's `run()` raised TRACE_INACTIVE and `record()` has the problem TRACE_INACTIVE; or (ii) `run()` returned f's value, `record()` has no problem, A's end is `"returned"` or `"open"`, and A counts at most `{f:1}`. In every trial: after the join `anchors == 0`, `global_events == 0`, `clearing == 0`, and no MONITOR_LOST (f returns, so every MONITOR_LOST here is false). Revision 3 fails it: `_ANCHORS[None]` in 51–62 of 20,000 natural races per version, and a false MONITOR_LOST in 17–23 (`rev4/b1_open_races_exit.py`) | #20's outcome set; B1, MF2 (rev. 4) |
| X148 | revision 4, MF2; main thread before the self-trace. T1 runs `cov.run('A', f)`. The id-3 tool, PY_START on `_ours` filtered to T1, holds T1 at its first `_ours` call after A's body returned (in its close's `UNWIND_LOST` test) until the main thread reaches PY_START of `_v5_faultpoints()['_exit_txn']` (X5, after X3 popped A and cleared the event); there the tool holds the main thread until T1's PY_RETURN from `_detach` | A PASS `{f:1}`, no MONITOR_LOST. With `_detach` testing the anchor before the event (revision 3's order), T1 passes the anchor test before its hold, reads the event clear after it, and flags `UNWIND_LOST` before X5 reads the flags: MONITOR_LOST (`rev4/a4r_detach_order.py`, `rev4/w4_rev4_witnesses.py`, both versions) | `_detach`'s test order (rev. 4) |
| X149 | revision 4, MF1; main thread before the self-trace. T2 closes section B, the last anchor. The id-5 injector raises `Injected` on T2 at the first instruction of `_unwind_off` at which `_v5_state()["clearing"]` is 1 (after the announcement, before the test). Then T1 runs `cov.run('A', catch_t)` | `Injected` from T2's `run()`; right after it `clearing == 1` and `global_events` is PY_UNWIND. T1's `run()` returns within 1 s, A PASS `{t:1}`, no MONITOR_LOST, and `clearing == 0` after it. With `_await_clearers` waiting on dead tokens (no `_alive` test), T1's `run()` raises MACHINERY_BUSY after 10 s (the model: after its shortened bound) | the liveness test in `_await_clearers` (rev. 4) |
| X149b | revision 4, MF1; main thread before the self-trace. X149's fault, then no open: a second tracer declaring g is entered and exited | `clearing == 1` after the fault, 0 after the second tracer's transaction. Without the reconciliation's announcement sweep, it stays 1 | the announcement sweep (rev. 4) |
| X150 | revision 4, MF1, I5; main thread before the self-trace; the upper boundary row of the opener's wait. X145 (a)'s hold of T2 at the clear, but released **11 s after** T1 reaches PY_START of `_await_clearers` (measured from that moment, as X138 measures) | T1's `run()` raises MACHINERY_BUSY after waiting at least 9.5 s, recorded; T2's close completes; afterwards `anchors == 0`, `clearing == 0`; a new section then PASS | the busy bound at open, raised (rev. 4) |
| X142b | revision 4, N1; in a fresh subprocess. Before any tracer, another tool takes id 4 with `use_tool_id(4, N('other'))`, where `N` is a str subclass whose `__eq__` returns True and counts its calls. Then a tracer on f runs a section calling f | PASS `{f:1}`; `_v5_state()["tool"] == 3`; the `__eq__` counter is 0; id 4 still carries the other tool's name with its callbacks untouched. With `_named` comparing without the exact-type test, styxx adopts id 4 as its own (`tool == 4`) and the counter is above 0 (`critic4/a5_tool_name_eq.py`: `use_tool_id` accepts the subclass and its `__eq__` returns True, both versions) | `_named` (rev. 4) |
...
| V66 | revision 4: X150 with the hold released **7 s** after T1 reaches PY_START of `_await_clearers` (the lower boundary row of the opener's wait) | T1's open succeeds after waiting at least 6 s; A PASS `{t:1}`; no MONITOR_LOST. A bound of 0.5 s, ×0.5 or ×2 for the opener's wait fails V66 or X150, as for the mutex's bound (V65, X138) | the busy bound at open, lowered (rev. 4) |
...
| revision 3: `_commit` inside `_run`'s try (moved back before it) / `o.armed` in `_detach`'s check / the anchor test in `_detach`'s check / `_unwind_off()` at the end of every reconciliation / `_unwind_off` in revision 2's order (test first, no announcement) / `_retire` step 6 is `_unwind_off()` (revision 2's test-then-clear) | X144 / X143b / X143c / X143c / X145 (a) / X145 (b) |
| revision 4: `o.frame` released by every `_detach` (revision 3's) / step 8's re-check deleted / its `'exiting'` test deleted / its claim test deleted / the anchor sweep deleted / `_unwind_off` without its announcement / the announcement withdrawn before the test / `_commit` without `_await_clearers` / `_unwind_on` before the wait / no `_unwind_on` in `_commit` / `_detach`'s test with the anchor before the event / no `_alive` test in `_await_clearers` / the announcement sweep deleted / `_named` without its exact-type test / the opener's busy bound raised, lowered | X146 / X146 / X146b / X146c / X146d / X145 (a) / X145 (a) / X145 (a) / X145 (a) / X143 / X148 / X149 / X149b / X142b / X150, V66 |
...
- revision 3: `_commit` moved before `_run`'s try (X144); `o.armed` dropped from `_detach`'s check (X143b); the anchor test dropped from it (X143c); the reconciliation's final `_unwind_off()` deleted (X143c); `_unwind_off` in revision 2's order (X145 (a)); `_retire` step 6 as revision 2's test-then-clear (X145 (b)). Each was checked on `rev3/mech3.py`: the witness's outcome differs under the mutant, on both versions (`rev3/w1_rev3_witnesses.py`), and again on `rev4/mech4.py` for the revision-4 forms of the witnesses (`rev4/w4_rev4_witnesses.py`). SM1 admission re-checks it on `ref_v5f.py`;
- revision 4: every row of the revision-4 line of the Mutation-audit table, each with the witness named there. All except the `_named` and busy-bound rows were checked on `rev4/mech4.py`: the witness's outcome differs under the mutant, on both versions (`rev4/w4_rev4_witnesses.py`, 18 mutant rows). The `_named` row rests on `critic4/a5_tool_name_eq.py`. The busy-bound rows rest on `rev2/p12_busy_bound.py`'s model of the same wait. `rev4/m1_pairs_modelcheck.py mutants` also finds a violation for each of the 11 model-level mutants in the pair enumeration;
...
- **G_FI (crash sweep, frozen).** Required on 3.12 and 3.13, on two threads: every fault point is **clean**. Clean is defined by outcome alone, so no window has to be mapped to implementation offsets. A point is clean iff C1–C8 below hold, and C9 as well in the fork scenario. Revision 2's clauses (a)–(e) are C3, C5 with C7, C8, C8's second reading, and C2.
  ...
  *Vocabulary.*
  - The *scenario* is the frozen program run at a point. It constructs tracers only through `coverage_trace()`, catches nothing, and so stops at the first exception that reaches it. The *faulted tracers* are the tracers it constructed. Revision 4 (MF3): every scenario joins every thread it started before any tracer's `__exit__`, so no open or close in it runs concurrently with an exit, and its baseline record is deterministic. Races between concurrent transitions are covered by `m1_pairs_modelcheck` (Revision 4 table), X145–X150 and X147, not by G_FI.
  ...
  *The invariants:*
  ...
  - **C5, nothing left once the faulted tracers are gone.** After step 6, `_v5_state()` equals `S0` except that `cut` may have grown, `tool` may have been set, and, in the background variant, the `pending` field of the background tracer's target mint is not compared. That field can differ fault-free (revision 4, MF6). A frame of the target entered while any anchor is registered anywhere stores a pending entry. If it raises after the last anchor closed, S is clear and nothing pops the entry until the mint retires, which the background tracer's hold prevents during the sweep (`critic4/a6_pending_left_fault_free.py`: 1 entry left with `anchors` 0 and `global_events` 0, both versions). C4 checks that it is gone once the background tracer exits. In particular:
    ...
    - `anchors` is 0, and `clearing` is 0 (revision 4);
    - `guard` is `"free"`;
...
- **G_HYG (static).** These must hold:
  ...
  - (revision 2) `_forget_in_child` writes `__code__` only in its last loop, after every registry and event step; `_detach` calls `_unwind_off()` after its anchor pop;
  - (revision 4, replacing revision 3's two clauses) `_commit`'s statements are, in order: the anchor store `_ANCHORS[o.frame] = o`; the step-8 test of `'fin' in o.fin` or `'exiting'` (with `_detach`, the TRACE_INACTIVE append and the raise); `if _CLEARING: _await_clearers(o)`; `_unwind_on()`; `o.armed = True`. `_open` stores no anchor and calls no `_unwind_on`. `_unwind_on` is called only from `_commit`;
  - (revision 4) `_unwind_off`'s statements are, in order: the `_ours()`/`get_events` early return; `me = _txn()`; `_CLEARING[me] = True`; the `_ANCHORS` test with the clear `set_events(_TOOL[0], 0)` as its only branch; `_CLEARING.pop(me, None)`. Nothing requires the test and the clear to be one expression or one line (M7). `set_events` is called only in `_unwind_on`, `_unwind_off` and `_forget_in_child`; `_retire` step 6 is `_unwind_off()`; `_reconcile` ends with the anchor sweep, the announcement sweep and `_unwind_off()`, in that order;
  - (revision 4) `_detach` never assigns `o.frame`. `o.frame` is assigned only in `_open` and in `_run`'s and `_run_async`'s `finally`. `_detach`'s `UNWIND_LOST` test reads `o.armed`, then `_ours()` and `get_events`, then `_ANCHORS.get(fr) is o`, in that order;
  - (revision 4, N1) `_TOOL_NAME` is compared only inside `_named`, whose test is `type(n) is str and n == _TOOL_NAME`;
...
- **G_COVER.** Every executable line of the region runs in at least one of the four passes below. No pragma exempts a line. A line that no pass reaches is dead code and must be deleted. Revision 3 drops revision 2's "or folded onto its guard's line" (N12; see "Line coverage only").
  ...
  - **The four passes.** The tool is enabled at the start of every process each pass creates.
    ...
    Lines that the spec requires and that run only in a subprocess are reached in pass 1's subprocess cases: `_reclaim`'s body (X137 free variant), MONITOR_BUSY's raise (X36), and `_ensure_tool`'s id-3 fallback (X142, X137b). Revision 4: `_await_clearers` runs only while another thread is inside `_unwind_off`'s window, so its lines are reached in pass 1 by X145 (a) (the wait), X149 (a dead announcement popped) and X150 (the MACHINERY_BUSY raise); the reconciliation's two sweeps by X146d and X149b.
...
2. **L-DELIVERY as possible program corruption.** A PY_UNWIND callback that raises replaces the in-flight exception, and on 3.12.3 and 3.13.12 its `__context__` is None. The second critic answered round-5 question 2 with a probe: yes, an `except ValueError` flow in undeclared code loses its handler under a signal flood, at hundreds of thousands per 2M iterations. Revision 2 confines this to open sections: 0 outside them (`rev2/p4_unwind_scope.py`). It also freezes that G_SIG does not count it as a wrong result. Round 5 should attack what remains:
   ...
   - the per-section toggle. Revision 4 replaces revision 3's atomic one-line test-and-clear with an announcement and an opener's wait (M7), whose argument uses only the order of single C operations. Attack its premises and its costs: `_alive`'s exactness for a clearer's token (a frame id reused by a new `_unwind_off` frame, `f_locals` reads of another thread's frame on 3.12); the same-thread exclusion (R19) and greenlet switches; a clearer held by user code, which makes openers wait up to 10 s and then refuse MACHINERY_BUSY; and the anchor frame an opening now keeps until its owner's `finally` runs (M1). Also attack the dead-anchor bound (U4): a fault at a close's first instruction keeps the event set until that trace exits or is swept;
...
3. **The opener's wait (revision 4).** Revision 4 makes the unwind scope independent of atomicity: a clearer announces itself before its test, and an opener waits for announced clearers before it sets the event. That is the first wait outside the robust mutex, and it is bounded the same way (10 s, liveness by `_alive`, never on the opener's own thread). The alternative the fourth critic offered was to keep revision 3's one-line test-and-clear and state an exact list of instruments under which U1 fails. That list includes tools this spec says coexist: pure-Python profilers, cProfile with a Python timer, CALL tools, and BRANCH tools on 3.12. It would also have left MONITOR_LOST inside I6 and made C4's allowance vacuous. The wait costs about 1.3–1.7 µs per section in the model, uncontended (`rev4/b4_cost.py`). A contended open waits for as long as the clearer's window lasts: normally a few statements, and up to 10 s if user code holds the clearer.
