#!/usr/bin/env python
"""EXAM_HOLE (declaration parse; NOTE-level): SECTION_DECL refuses a `section` that is not a
non-empty ASCII str. Dropping the ASCII clause survives the frozen exam (X06, X07 test only a missing
`exercises` and `section: 5`). The original refuses SECTION_DECL for a non-ASCII section; the mutant
accepts it. (A sibling mutant that drops the non-empty clause survives the same way.)
...
MUTANT_NAME = "D01_section_non_ascii_accepted"
MUTATION = [('            if not isinstance(sec, str) or not sec or not sec.isascii():', '            if not isinstance(sec, str) or not sec:')]
...
HARNESS = r'''mod = module("rt4d01_fx", "def f(): return 1\n")
try:
    exp = experiment(G={"exercises": ["rt4d01_fx:f"], "section": "sección"})
    emit(parse="accepted")
except GateSpecError as e:
    emit(parse=str(e)[:80])'''
...
def judge(orig, mut):
    ok = orig["parse"].startswith("[V5:SECTION_DECL]") and mut["parse"] == "accepted"
    return ok, ("a non-ASCII section is refused SECTION_DECL by the original but accepted by the mutant"
