# Revision 8: the eighth critic's c1 (critic8/c1_del_between_exchange_and_count.py) run on rev8/mech8.py: 'spec' is
# revision 8's tee registration, 'rev7' is MUT 'reg_rev7' (revision 7's form). Otherwise unchanged.
...
REV7 = os.path.dirname(os.path.abspath(__file__))   # revision 8: rev8/, mech8.py
...
if __name__ == '__main__':
    if len(sys.argv) == 4:
        print(json.dumps(child(sys.argv[1], sys.argv[2], sys.argv[3]), default=repr)); sys.exit(0)
    for form in ('spec', 'rev7'):
        for mode in ('observe', 'greenlet'):
            for order in ('P-first',):
                p = subprocess.run([sys.executable, __file__, mode, order, form], capture_output=True, text=True, timeout=120)
                out = p.stdout.strip().splitlines()[-1] if p.stdout.strip() else 'ERROR ' + p.stderr[-800:]
                print(sys.version.split()[0], form, mode, order, out, flush=True)
