...
class Cfg:
    def __init__(self, name, threads, ncores, covers, intr=(), intr_on=None, ext=(), faults=(0, 1), I=0, X=0, Wsw=0,
                 facade_dead=(), not_entered=(), slots=None, note='', xrep=1, fresh=False):
...
class Model:
    ...
    def step(self, ths, sh, pk, k):
        ...
        if pc == 'X5N':
            ...
            if FIX_CB[0] and ok and not r[RGPOK]: sh2 = sset(sh2, LOSTN_, sh2[LOSTN_] + 1)   # critic7 fix: count a repaired callback
            m = sh2[MINT_]
            holders = m[3] - {c}
...
if __name__ == '__main__':
    ...
    if which == 'main':
        ...
        MUTS = ['off_split', 'popoff_split', 'on_ungated_own', 'on_no_capture', 'on_capture_split', 'on_capture_nogate',
                ...
                'blind_read_gated', 'ensure_nocount+blind_read_gated']
        sel = sys.argv[2:]
        if sel: MUTS = sel
        for mu in MUTS:                              # revision 6: every configuration, the triples included (N5)
            found = []
            for cfg in CONFIGS:
                for b in budgets(cfg, 6):
                    r = explore(cfg, 6, b, frozenset(mu.split('+')))
                    f = fmt(r)
